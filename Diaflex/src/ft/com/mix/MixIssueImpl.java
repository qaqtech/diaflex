...
public class MixIssueImpl implements MixIssueInterface{
  ...
  return gtTotalMap ;
    ...
    public ArrayList SearchResult(HttpServletRequest req ,HttpServletResponse res, String vnmLst ){
      ...
      util.setLogApplNm(info.getLoApplNm());
        ...
        String rsltQ = srchQ +fldSh +" sh ,"+fldMemo+" memo ,"+fldSize+" sz  from gt_srch_rslt where flg =? group by "+fldSh+","+fldMemo+","+fldSize;
  ...
  public void LabIssueEdit(HttpServletRequest req,HttpServletResponse res){
    ...
    util.setLogApplNm(info.getLoApplNm());
      ...
      String labIssueEdit ="select b.mprp from mprc a , prc_prp_alw b " + 
                           "where a.idn = b.prc_idn and a.is_stt='LB_IS'and b.flg='ISSEDIT'";
      ArrayList  rsLst = db.execSqlLst("labIssue", labIssueEdit, new ArrayList());
      PreparedStatement  stmt =(PreparedStatement)rsLst.get(0);
      ResultSet  rs =(ResultSet)rsLst.get(1);
        try {
            while (rs.next()) {
...
//        init(req,res);
//        ArrayList asViewPrp = (ArrayList)session.getAttribute("MIXISGNCSrch");
//        try {
//            if (asViewPrp == null) {
//
//                asViewPrp = new ArrayList();
//                ResultSet rs1 =
//                    db.execSql(" Vw Lst ", "Select prp,flg  from rep_prp where mdl = 'MIX_IS_SRCH' and flg in ('Y','S') order by rnk ",
//                               new ArrayList());
//                while (rs1.next()) {
...
//                }
//                rs1.close();
//                session.setAttribute("MIXISGNCSrch", asViewPrp);
//            }
//
//        } catch (SQLException sqle) {
//            // TODO: Add catch code
