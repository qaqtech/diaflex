...
public class MixInwardReportAction  extends DispatchAction {
    ...
    public ActionForward fetch(ActionMapping am, ActionForm af, HttpServletRequest req,
      ...
      if(!rtnPg.equals("sucess")){
          ...
          String inwardSql="select 'MFG' typ, b.srt, b.val , m.dsc,to_char(a.dte,'dd-MON-yyyy') dte, to_char(sum(a.cts),'999990.00') cts , sum(a.qty) qty , \n" + 
          "trunc(((sum(trunc(a.cts,2)*nvl(a.upr, a.cmp)) / sum(trunc(a.cts,2))))) avg_Rte\n" + 
          "from mstk a , stk_dtl b , stk_dtl c,prp m\n" + 
          "where a.idn=b.mstk_idn and a.pkt_ty='MIX' and b.mprp='BOX_TYP' and b.grp=1 and\n" + 
          "b.mstk_idn=c.mstk_idn and c.mprp='RECPT_DT' and c.grp=1\n" + 
          "and m.mprp='BOX_TYP' and m.val=b.val \n" +conQ+ 
          " and not exists (select 1 from pur_dtl p where p.mstk_idn=a.idn )  and a.cts > 0 \n" + 
          "group by b.val , b.srt ,m.dsc,a.dte\n" + 
          "union\n" + 
          "select 'PUR' typ, b.srt, b.val ,m.dsc,to_char(a.dte,'dd-MON-yyyy') dte, to_char(sum(a.cts),'999990.00') cts , sum(a.qty) qty , \n" + 
          "trunc(((sum(trunc(a.cts,2)*nvl(a.upr, a.cmp)) / sum(trunc(a.cts,2))))) avg_Rte\n" + 
          "from mstk a , stk_dtl b , stk_dtl c,prp m\n" + 
          "where a.idn=b.mstk_idn and a.pkt_ty='MIX' and b.mprp='BOX_TYP' and b.grp=1 and\n" + 
          "b.mstk_idn=c.mstk_idn and c.mprp='RECPT_DT' and c.grp=1\n" + 
          "and m.mprp='BOX_TYP' and m.val=b.val \n" +conQ+ 
          "and  exists (select 1 from pur_dtl p where p.mstk_idn=a.idn )  and a.cts > 0\n" + 
          "group by b.val , b.srt,m.dsc,a.dte \n" + 
          "order by 1,2";
