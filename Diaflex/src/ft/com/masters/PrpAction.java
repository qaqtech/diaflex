...
public class PrpAction extends DispatchAction {
   
    private final String formName   = "prpfrm";
    ...
    public ActionForward load(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
        ...
        if(!rtnPg.equals("sucess")){
            ...
            util.updAccessLog(req,res,"Prp form", "load start");
        ...
        udf.setValue("mprp", mprp);
            ...
            pageDtl=new HashMap();
            pageDtl=util.pagedef("PRP_FORM");
            allPageDtl.put("PRP_FORM",pageDtl);
            }
            util.updAccessLog(req,res,"Prp form", "load end");
    ...
    public ActionForward save(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
        ...
        util.updAccessLog(req,res,"Prp form", "save start");
                    HashMap  formFields = info.getFormFields(formName);
        ...
        if (udf.getAddnew() != null) {
            ...
            for (int i = 1; i <= lmt; i++) {
                ...
                ArrayList params   = new ArrayList();
                String insIntoQ = " insert into prp (mprp  ";
        ...
        if (udf.getModify() != null) {
            ...
            String    getMenu = "select " + helper.getSrchFields(daos, "mprp", false) + " from prp where mprp=? order by srt ";
            ArrayList    params  = new ArrayList();
            params.add(udf.getValue("mprp"));
            ...
            while (rs.next()) {
                ...
                String updQ  = " update prp set  ";
                String condQ = " where mprp = ? and val=?" ;
                boolean isValid = false;
...
//        util.initPrp();
        ...
        postRequest = new HttpPost(rep_path+"/diaflexWebService/REST/memservice/refresh");
        postRequest.setHeader("Accept", "application/json");
        postRequest.setHeader("Content-type", "application/json");
        jObj = new JSONObject();
        try{
           jObj.put("cnt", info.getDbTyp());
           jObj.put("method", "initPrpSrch");
        } catch (JSONException jsone) {
        jsone.printStackTrace();
        }
                     
        insetValue = new StringEntity(jObj.toString());
        insetValue.setContentType(MediaType.APPLICATION_JSON);
        postRequest.setEntity(insetValue);                       
        responsejson = httpClient.execute(postRequest);                       
        if (responsejson.getStatusLine().getStatusCode() !=200) {
        throw new RuntimeException("Failed : HTTP error code : "+responsejson.getStatusLine().getStatusCode());
        }                                                                           
        br = new BufferedReader(new InputStreamReader((responsejson.getEntity().getContent())));
        outsetValue="";
        jsonStr="";
        //System.out.println("OutsetValue from Server .... \n"+br.readLine());
        while ((outsetValue = br.readLine()) != null) {
        ...
        util.updAccessLog(req,res,"Prp form", "save end");
    ...
    public String init(HttpServletRequest req , HttpServletResponse res,HttpSession session,DBUtil util) {
            ...
            System.out.print("New Session Id :="+session.getId());
                ...
                rtnPg=util.checkUserPageRights("masters/propertyMst.do?","");
                if(rtnPg.equals("unauthorized"))
                util.updAccessLog(req,res,"Prp form", "Unauthorized Access");
                else
                util.updAccessLog(req,res,"Prp form", "init");
            }
            }
            return rtnPg;
         }
