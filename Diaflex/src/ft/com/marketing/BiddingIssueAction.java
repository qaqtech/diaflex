...
public class BiddingIssueAction extends DispatchAction {
  ...
  public ActionForward load(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
      ...
      if(!rtnPg.equals("sucess")){
          ...
          BiddingIssueFrom udf = (BiddingIssueFrom)af;
          udf.resetAll();
          ...
          pageDtl=new HashMap();
          pageDtl=util.pagedef("BIDDING_ISSUE");
          allPageDtl.put("BIDDING_ISSUE",pageDtl);
          }
    ...
    public ActionForward fetch(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
        ...
        if(!rtnPg.equals("sucess")){
          return am.findForward(rtnPg);   
            ...
            String lstNme = "BIDISS_"+info.getLogId()+"_"+util.getToDteGiveFrmt("yyyyMMdd");
            gtMgr.setValue(lstNme+"_SEL",new ArrayList());
            gtMgr.setValue(lstNme, stockList);
            gtMgr.setValue("lstNmeBIDISS", lstNme);
            if(stockList.size()>0){
       ...
       if(!vnm.equals("")){
          ...
          for(int i=1; i <= loopCnt; i++) {
            ...
            String lookupVnm = vnmLst[aryLoc-1];
              ...
              srchRefQ = 
               "    Insert into gt_srch_rslt (pkt_ty, stk_idn, vnm, pkt_dte, stt , flg , qty , cts , sk1 , rmk,rap_rte,rap_dis,cmp ,quot) " + 
               " select  pkt_ty, b.idn, b.vnm,  b.dte, stt , 'Z' , qty , cts , sk1 , tfl3,b.rap_rte,decode(b.rap_rte ,'1',null,'0',null, trunc((nvl(b.upr,b.cmp)/greatest(b.rap_rte,1)*100)-100, 2)),nvl(upr,cmp) ,nvl(upr,cmp) "+
               "     from mstk b "+
               " where  cts > 0  and stt = 'MKAV'";
    ...
    public ActionForward Issue(ActionMapping am,ActionForm form,HttpServletRequest req,HttpServletResponse res) throws Exception {
      ...
      if(!rtnPg.equals("sucess")){
        ...
        HashMap dbinfo = info.getDmbsInfoLst();
        int stepCnt = Integer.parseInt((String)dbinfo.get("STEPCNT"));
          BiddingIssueFrom udf = (BiddingIssueFrom)form;
        ...
        String prcId = (String)udf.getValue("mprcIdn");
        String empId = (String)udf.getValue("empIdn");
        ...
        issueIdn = cst.getInt(3);
         ...
         if(ct>0){
             for(int i=0;i<stkIdnLst.size();i++){
                 ...
                 String newPrc = (String)udf.getValue("nwprice_"+stkIdn);
                 String newdis = (String)udf.getValue("nwdis_"+stkIdn);
                 if(!newPrc.equals("") && !newdis.equals("")){
                     newPrc = util.roundToDecimals2(newPrc, 2);
    ...
    public HashMap SearchResult(HttpServletRequest req ,HttpServletResponse res, String vnmLst ){
      ...
      util.setLogApplNm(info.getLoApplNm());
       init(req,res,session,util);
          ...
          String gtMemoPri =
             " insert into gt_memo_pri_chng(memo_id, exh_rte, mstk_idn, cts, rap_rte, rte, quot) "+
              " select 100, nvl(a.exh_Rte, 1), a.stk_idn, trunc(a.cts, 2) cts, a.rap_rte , a.cmp , a.cmp "+
            " from gt_srch_rslt a where flg='Z'";  
                  
          ct = db.execUpd(" ins gt_memo_pri_chng", gtMemoPri,new ArrayList()); 
          
                   
      
        ArrayList vwPrpLst = mktPrprViw(req,res);
        String  srchQ =  " select stk_idn , pkt_ty,  vnm, pkt_dte, stt , qty , cts ,quot, rmk,decode(rap_rte, 1, '', decode(least(rap_dis, 0),0, '+'||rap_dis, rap_dis))  r_dis ,to_char(cts * quot, '99999990.00') amt,to_char(cts * rap_rte, '99999990.00') rapvlu   ";
        ...
        try {
            while(rs.next()) {
                ...
                pktPrpMap.setValue("USDVAL",util.nvl(rs.getString("amt")));
    ...
    public ArrayList getPrc(HttpServletRequest req ,HttpServletResponse res,DBMgr db){
        ...
        ArrayList prcList = new ArrayList();
          ...
          String prcSql = "select idn, prc , in_stt from mprc where  stt = ? and grp='BID' " ;
              if(!grp.equals("")){
    ...
    public String init(HttpServletRequest req , HttpServletResponse res,HttpSession session,DBUtil util) {
            ...
            util.updAccessLog(req,res,"Bidding Issue", "Unauthorized Access");
            else
            util.updAccessLog(req,res,"Bidding Issue", "init");
            }
            }
            return rtnPg;
    }
