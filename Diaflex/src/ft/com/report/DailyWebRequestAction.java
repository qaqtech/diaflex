...
public class DailyWebRequestAction extends DispatchAction {
    ...
    public ActionForward load(ActionMapping am, ActionForm form,
        ...
        util.updAccessLog(req,res,"Daily Web Request Report", "load start");
        DailyWebRequestForm dailyWebRequestForm = (DailyWebRequestForm)form;
        ResultSet rs = null;
        ...
        if(flg.equals("New")){
            dailyWebRequestForm.reset();
        }
        String dfr = util.nvl((String)dailyWebRequestForm.getValue("dtefr"));
        String dto = util.nvl((String)dailyWebRequestForm.getValue("dteto"));
        String spersonId = util.nvl((String)dailyWebRequestForm.getValue("styp"));
        String buyerId = util.nvl(req.getParameter("nmeID"));
        String group = util.nvl((String)dailyWebRequestForm.getValue("group"));
        String typ = util.nvl((String)dailyWebRequestForm.getValue("typ"));
        String sql="";
        String bql="";
      ...
      ", decode(a.typ,'O','PBB Issue','BID','Bid Issue','K','KSelect', 'I', 'Internal','H','Happy Hours', 'E', 'External', 'WH', 'Web Hold','S', 'Show', 'Z', 'Web Request') dsp_typ"+
      ...
      " from jan_v a, jandtl b,Nme_V c where a.idn = b.idn and c.Nme_Idn=A.Nme_Idn and trunc(a.dte) between "+dtefrom+" and "+dteto+" and a.typ in ('Z') and b.stt in ('RT') "+sql+""+bql+
      ...
      req.setAttribute("Display_typ", Display_typ);
            ...
            HashMap pageDtl=(HashMap)allPageDtl.get("DAILY_WR_RPT");
            if(pageDtl==null || pageDtl.size()==0){
            pageDtl=new HashMap();
            pageDtl=util.pagedef("DAILY_WR_RPT");
            allPageDtl.put("DAILY_WR_RPT",pageDtl);
            }
            info.setPageDetails(allPageDtl);
            util.updAccessLog(req,res,"Daily Web Request Report", "load end");
            util.saleperson();
            if(cnt.equals("hk"))
            util.groupcompany();
    ...
    public ActionForward pktDtlExcel(ActionMapping am, ActionForm form,
        ...
        if(!rtnPg.equals("sucess")){
            ...
            util.updAccessLog(req,res,"Daily Web Request Report", "pktDtlExcel start");
            GenericInterface genericInt = new GenericImpl();
        ArrayList vwprpLst = genericInt.genericPrprVw(req,res,"DAILY_WR_VW","DAILY_WR_VW");
        DailyWebRequestForm  dailyWebRequestForm  = (DailyWebRequestForm )form;
       ...
       String dteto = " trunc(sysdate) ";
        if(!nmeidn.equals("") && !dte.equals("")){
         ...
         " and exists (select 1 from rep_prp rp where rp.MDL = 'DAILY_WR_VW' and st.mprp = rp.prp)  And st.Grp = 1) " +
        ...
        util.updAccessLog(req,res,"Daily Web Request Report", "pktDtlExcel end");
