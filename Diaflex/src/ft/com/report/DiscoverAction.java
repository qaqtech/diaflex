...
public class DiscoverAction extends DispatchAction {
    ...
    public ActionForward load(ActionMapping am, ActionForm form,
        ...
        dis.reset();
            ArrayList disSrchList = genericInt.genricSrch(req,res,"DIS_SRCH","DIS_SRCH");
    ...
    return am.findForward("load");
     ...
     String applybydis=util.nvl((String)dis.getValue("applybydis"));
             ArrayList genricSrchLst = genericInt.genricSrch(req,res,"DIS_SRCH","DIS_SRCH");
     ...
     if(pkt_ty.equals("ALL")){
         ...
         if(!dflt_val.equals("ALL") && !dflt_val.equals("DEL"))
     ...
     if(vnm.equals("")){
     if(disStt.length>0 || pkttyLst.contains("DEL")){
     String stt="";
     if(disStt.length>0){
     stt=disStt[0].trim();
     stt=disStt[0];
...
//         }
         ...
         int delct = db.execUpd("delete gt", "delete from gt_srch_rslt where stt='PROINV'", new ArrayList());
     ...
     String srchQ = " select stk_idn , pkt_ty, vnm, pkt_dte,prte,mrte, stt , qty ,cmnt,cert_no,rmk tfl3, cts ,kts_vw kts, quot ,rap_rte,decode(rap_rte, 1, '', trunc(((nvl(quot,0)/greatest(rap_rte,1))*100)-100,2)) dis,trunc(quot*cts,2) total_val, nvl("+prirapPrp+",prte) priraprte ,trunc(((greatest(nvl("+prirapPrp+",prte),1)*100)/greatest(rap_rte,1)) - 100,2) prirapdis,trunc(rap_rte*cts,2) rap_val "+conQ;
   ...
   public ActionForward createXL(ActionMapping am,ActionForm form,HttpServletRequest req,HttpServletResponse res) throws Exception {
       ...
       if(!rtnPg.equals("sucess")){
           ...
           util.updAccessLog(req,res,"Discover Report", "createXL start");
       ...
       HSSFWorkbook hwb = xlUtil.getDataAllInXl("SRCH",req,"DIS_VIEW");
       hwb.write(out);
       out.flush();
       out.close();
           util.updAccessLog(req,res,"Discover Report", "createXL end");
      return null;
       }
   }
