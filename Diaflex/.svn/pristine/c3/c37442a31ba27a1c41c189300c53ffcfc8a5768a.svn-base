...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <%
   ArrayList params = new ArrayList();
    ...
    String formAction = "/dashboard/propertyblock.do?method=save";
 %>
 <tr>
...
<%}%>
  ...
  <html:hidden property="value(pgidn)" />
    <table class="grid1">
...
<%
    ...
    for(int dsp=0; dsp < dspOrder.size(); dsp++) {
      ...
      %>
        <html:checkbox property="<%=cboxPrp%>"  name="userrightsform"/>
      <%}%>
            </td>
      <%
        for(int j=0; j < daos.size(); j++) {
        ...
        <%
            if(fldTyp.equals("T")) {
            %>
                <html:text property="<%=fldNm%>" name="userrightsform"  size="<%=fldSz%>"/>
            <%}
            if((fldTyp.equals("S")) || (fldTyp.equals("L"))){
                ...
                %>
                <html:select property="<%=fldNm%>"  name="userrightsform">
             ...
             if(fldTyp.equals("DT")){%>
            <div class="float">   <html:text property="<%=fldNm%>"  name="userrightsform"  styleId="<%=htmlFld%>" /></div>
      ...
      if(msg.equalsIgnoreCase("NEW")){%>
        ...
        <%}else {
            ...
            String link = info.getReqUrl() + "/dashboard/propertyblock.do?method=delete&idn="+ fldId ;
