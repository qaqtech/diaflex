...
<html>
 ...
 %>
  ...
  <img src="../images/line.jpg" border="0" /></td><td valign="middle"> <span class="pgHed">Mixing Final Return Matrix</span> </td>
</tr></table>
  </td>
  </tr>
 <%String msg = (String)request.getAttribute("msg1");
   ...
   <html:form action="mix/mixFnlRtnAction.do?method=loadGrid" method="post"  onsubmit="return validateRtn_memo()" enctype="multipart/form-data">
  <table  class="grid1">
  <tr><th colspan="2">Generic Search </th></tr>
  <tr><td align="center">Process</td><td align="center">
    <html:select property="value(mprcIdn)"  styleId="mprcIdn"  name="mixFnlRtnForm"   >
       <html:optionsCollection property="prcList"  name="mixFnlRtnForm"
            label="prc" value="mprcId" />
    </html:select>
   </td> </tr>
   <tr><td align="center">Issue Id</td><td align="center"><html:text property="value(issueId)" styleId="issueId" name="mixFnlRtnForm" /> </td> </tr>
  ...
  <tr>
   <td colspan="2">
    ...
    <tr><td>Load file in CSV</td>
        <td><html:file property="mixFile"  name="mixFnlRtnForm"/> </td>
      </tr> 
  ...
  <tr> <td valign="top" class="tdLayout">
  <html:form action="mix/mixFnlRtnAction.do?method=save" method="post" onsubmit="return confirmChanges()" >
  <input type="hidden" name="lockId" id="lockId" value="" />
  <html:hidden property="value(srchId)" styleId="srchIdn" name="mixFnlRtnForm" />

  <html:hidden property="value(SHAPE)" styleId="sh" name="mixFnlRtnForm" />
  <html:hidden property="value(INSTT)" styleId="instt" name="mixFnlRtnForm" />
  <html:hidden property="value(ISSTT)" styleId="isstt" name="mixFnlRtnForm" />
  <html:hidden property="value(OTSTT)" styleId="otstt" name="mixFnlRtnForm" />
  <%
  ...
  if(view.equals("Y")){
  if((sizeClrMap!=null && sizeClrMap.size()>0) && (maxCrtMap!=null && maxCrtMap.size()>0)){
 ...
 <td><input type="button" name="reset"  class="submit" value="Enable Edit" onclick="UnabledText()" id="reset" style="display:none"/></td>
   <td><input type="button" name="verifySelected" style="display:none" class="submit" onclick="MixFnlVerify()" value="Verify Changes" id="verify"/></td>
  <td><input type="button" name="unlock" style="display:''" class="submit" onclick="MixFnlUnlock()" value="UnLock" id="unlock"/></td>
  ...
  if(dataRte!=null && dataRte.size()>0)
   ...
   String onChange = "saveSizeClrTextFnl("+mixClrsrt+","+mixSizesrt+",'0','"+styleCts+"', 'DP_0' ,this)";
  String  onChangePri = "saveSizeClrPriTextFnl("+mixClrsrt+","+mixSizesrt+",'0','"+styleCts+"', 'DP_0','"+rte+"',this)";
  ...
  String fldValRte = "value(PRI_"+fldName+")";
   ...
   onChange = "saveSizeClrTextFnl('"+mixClrsrt+"','"+mixSizesrt+"','"+dpVal+"','"+styleCts+"' ,'DP_"+dpDsc+"','"+rte+"',this)";
   onChangePri = "saveSizeClrPriTextFnl('"+mixClrsrt+"','"+mixSizesrt+"','"+dpVal+"','"+styleCts+"' ,'DP_"+dpDsc+"','"+rte+"', this)";
   keypresscts="keypressMix(this.id,'"+nextmixClrsrt+"','CTS',event)";
   keypressPri="keypressMix(this.id,'"+nextmixClrsrt+"','PRI',event)"; 
   String tbox=util.nvl((String)sizeClrMap.get(mix_clarity+"_"+mix_Size));
   if(tbox.equals("T")){
   %>
    <td><html:text  property="<%=fldVal%>" disabled="true"  styleId="<%=styleCts%>" name="mixFnlRtnForm" size="10" onchange="<%=onChange%>"  onkeypress="<%=keypresscts%>"/>
  <label class="red">   <bean:write property="<%=fldValIS%>" name="mixFnlRtnForm" /> </label> <%=rte%><div style="float:right;"><html:text  property="<%=fldValRte%>" disabled="true"   styleId="<%=styleRte%>" onchange="<%=onChangePri%>" name="mixFnlRtnForm" size="10"  value="<%=rte%>"  onkeypress="<%=keypressPri%>"/></div></td>
  <%}else{%><td></td> <%}%>
   <%}%>
   <% }else{
   String tbox=util.nvl((String)sizeClrMap.get(mix_clarity+"_"+mix_Size));
  ...
  <%}%>
   ...
   <td><%=pktDtl.get("dp")%></td>
    <td><%=pktDtl.get("upr")%></td>
   <td><%=pktDtl.get("cmp")%></td>
   </tr>
   <%}%>
