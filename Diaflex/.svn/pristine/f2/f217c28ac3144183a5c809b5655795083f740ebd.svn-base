...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <%
   ArrayList params = new ArrayList();
    ...
    String formName = "rapSyncMprpForm";
...
<%}%>
 ...
 <html:form action="masters/rapsyncmprp.do?method=save" method="post">
    <table class="grid1" >
      ...
      <%
        ...
        <%
           ...
           if(fldTyp.equals("DT")){%>
            ...
            <div class="float">     
                <img src="<%=info.getReqUrl()%>/images/calender.png"  onClick="setYears(1900, <%=info.getCurrentYear()%>);showCalender(this, '<%=htmlFld%>');" border="0" /></div>
            <% }
            %>
        </td>    
        <%}%>
       <% if(msg.equalsIgnoreCase("NEW")) {%>
        ...
        <%} else {
           if (fldId.indexOf("&") > -1) {
                fldId = fldId.replaceAll("&", "%26");
            }
            String link1="/masters/rapsyncprp.do?method=load&mprp="+ fldId ;
        %>
        <td nowrap="nowrap"><html:link page="<%=link1%>" target="frame">Add Properties </html:link></td>
        <%}%>
