...
<html>
  <head>
    ...
    <meta http-equiv="refresh" content="30;url=<%=info.getReqUrl()%>/Inward/transferMktSH.do?method=status&seq=<%=seqNo%>">
...
<input type="hidden" name="repPath" id="repPath" value="<%=dbinfo.get("REP_PATH")%>"/>
  ...
  <tr><td>Page Refreshed : </td><td><%=util.getDtTm()%></td> <td> <A href="<%=info.getReqUrl()%>/Inward/transferMktSH.do?method=status&seq=<%=seqNo%>"><img src="../images/refresh.gif" border="0"/></a> </td></tr>
  <tr><td colspan="3">
