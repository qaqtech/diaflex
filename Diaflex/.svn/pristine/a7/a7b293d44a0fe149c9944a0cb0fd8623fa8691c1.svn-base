...
<%}%>
 ...
 <html:form action="masters/country.do?method=save" method="post">
    <table class="grid1" >
    ...
    for(int dsp=0; dsp < dspOrder.size(); dsp++) {
      ...
      <%
       ...
       <% if(msg.equalsIgnoreCase("NEW")) {%>
        <td>&nbsp;</td>
        <%} else {
            String link1="/masters/city.do?method=load&countryIdn="+ fldId ;
        %>
        <td><html:link page="<%=link1%>" target="frame">Add Cities to Country </html:link></td>
        <%}%>
