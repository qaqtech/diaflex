...
public class LabIssueAction extends DispatchAction{
    ...
    ArrayList assortSrchList  =((ArrayList)new JspUtil().getFromMem(info.getMem_ip(),info.getMem_port(),info.getCnt()+"_lbISGNCSrch") == null)?new ArrayList():(ArrayList)new JspUtil().getFromMem(info.getMem_ip(),info.getMem_port(),info.getCnt()+"_lbISGNCSrch"); 
...
//    }
    ...
    ArrayList genricSrchLst  =((ArrayList)new JspUtil().getFromMem(info.getMem_ip(),info.getMem_port(),info.getCnt()+"_lbISGNCSrch") == null)?new ArrayList():(ArrayList)new JspUtil().getFromMem(info.getMem_ip(),info.getMem_port(),info.getCnt()+"_lbISGNCSrch"); 
    ...
    if(vnm.equals("")){
        ...
        stockList = labInt.SearchResultGT(req,res,"", labForm,"NORMAL");
    }else{
        stockList = labInt.insertgt(req,res,labForm,"NORMAL");
    }
    ...
    gtMgr.setValue("lstNmeLAB", lstNme);
    if(stockList.size()>0){
    ...
    labInt.LabIssueEdit(req, res,"LB_IS");
    }
    ...
    ArrayList labList = labInt.getLab(req,res);
    labForm.setLabList(labList);
    String labPrc=util.nvl((String)labForm.getValue("LAB_PRC_1"));
    if(!labPrc.equals("")){
                ArrayList lprpS = (ArrayList)prp.get("LAB_PRCS");
                ArrayList lprpV = (ArrayList)prp.get("LAB_PRCV");
                String lab = (String)lprpV.get(lprpS.indexOf(labPrc));
                lab = lab.trim();
                labForm.setValue("labIdn", lab);
     }
    labForm.setView("");
    labForm.setViewAll("");
    ...
    public ActionForward Issue(ActionMapping am,ActionForm form,HttpServletRequest req,HttpServletResponse res) throws Exception {
         ...
         boolean isDone = false;
          ...
          rs.close(); pst.close();
              ...
              outLst = db.execSqlLst("mprcId", "select min(idn) from mprc where in_stt='LB_AV' and stt='A' and grp <> 'ODD'", new ArrayList());
          ...
          for(int i=0;i< selectstkIdnLst.size();i++){
              ...
              if(isChecked.equals("yes")){
                  if(issueIdn==0){
                    ...
                    cst.close();
                    cst=null;
                      String globalrmk = util.nvl((String)labForm.getValue("rmk"));
                      if(!globalrmk.equals("") && issueIdn!=0){
                      params = new ArrayList();
                      params.add(globalrmk);
                      params.add(String.valueOf(issueIdn));
                      String updateFlg = "update iss_rtn set rmk=? where iss_id=?";
                      int cntmj = db.execUpd("update iss_rtn", updateFlg, params);
                      }
                  }
                  params = new ArrayList();
                  params.add(String.valueOf(issueIdn));
