...
<table class="popUpTb">
 <tr><td height="5%" id="close">&nbsp;&nbsp;&nbsp;<html:button property="value(close)" styleClass="submit"  onclick="MKEmptyPopup('SC');disablePopupASSFNL()" value="Close"  /> </td>
  <td height="5%" id="closereload" style="display:none">&nbsp;&nbsp;&nbsp;<html:button property="value(close)" styleClass="submit"  onclick="MKEmptyPopup('SC');disablePopupASSFNL();reportUploadclose('TALLY')" value="Close"  /> </td></tr>
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <%}}%>
   ...
   <table><tr><td>
   <html:form action="marketing/newStockTally.do?method=fetch" method="post" >
  <table  class="grid1">
 ...
 <% for(int n=0;n<sttList.size();n++){
  ...
  <tr> <td colspan="2"> <jsp:include page="/genericSrch.jsp"/>
   ...
   <tr><td colspan="2" align="center"><html:submit property="value(srch)" value="Issue Stock Tally" styleClass="submit" /></td> </tr>
   </table>
   </html:form>
   </td></tr>
 ...
 <html:option value="0" >---select---</html:option>
  ...
  %>
  <td>&nbsp;&nbsp;<a href="<%=info.getReqUrl()%>/marketing/newStockTally.do?method=dayWisehistory&reqstt=ALL" id="LNK_0" onclick="loadASSFNL('SC_0','LNK_0')"  target="SC"><%=fld_ttl%></a></td>
  <%}}%>
  ...
  if(pageList!=null && pageList.size() >0){
      ...
      fld_ttl=(String)pageDtlMap.get("fld_ttl");
  %>
  <td><input type="button" id="LNK_1" class="submit" onclick="purlab('LNK_1','SC','TALLY');"  value="Delete Sequence"/></td>
  <%}}%>
  </tr>
  </table>
  </td>
...
<%
pageList=(ArrayList)pageDtl.get("ISSUEGRAPH");
if(pageList!=null && pageList.size() >0){
if(seqList!=null && seqList.size()>0){
