...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
  <%
   ArrayList params = new ArrayList();
    ResultSet rs = null;
    util.updAccessLog(request,response,"Price Grp prp defform", "Price Grp prp defform");
    String formName = "pricegpprpdefform";
    
    HashMap allFields = (HashMap)info.getFormFields();
    HashMap formFields = (allFields.get(formName) == null)?util.getFormFields(formName):(HashMap)allFields.get(formName);
    ...
    String keyNme = (String)info.getValue("property");
    if(pgTtl.indexOf("~") > -1)
      pgTtl = pgTtl.replaceFirst("~nme", keyNme);
      
    util.SOP(" View "+ formName + " : fields = " + daos.size());
    int addRec = Integer.parseInt(util.nvl(uiForms.getREC(),"10"));
    String formAction = "/pricing/pricegpprpdef.do?method=save";
 %>
 <tr>
        ...
        <%
            ...
            if((fldTyp.equals("S")) || (fldTyp.equals("L"))){
                ...
                if(lovFormFlds.get(lovKey) != null) {
                    ...
                    if(lovQ.length() > 0) {
                        if(fldTyp.equals("S")) {
                            lovQ=lovQ.replace("?", "'"+keyNme+"'");;
                            lovKV = util.getLOV(lovQ);
                            }
                        else
                            lovKV = util.getLOVList(lovQ);
