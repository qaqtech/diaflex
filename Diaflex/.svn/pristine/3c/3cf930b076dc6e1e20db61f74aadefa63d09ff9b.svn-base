...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <tr>
    <td height="5" valign="top" class="greyline"><img src="ur/&lt;rnfo.getReqUrl()øimages/grey_linebig.png" width="685" height="5" align="right" /></td>
  </tr>
   <%
   int accessidn=0;
  ...
  <%
   ArrayList params = new ArrayList();
    ...
    String method = util.nvl((String)request.getAttribute("method"));
     String hideShowStatus = util.nvl((String)request.getAttribute("hideShowStatus"));
     String pur="";
     String search="";
     if(hideShowStatus.equals("PUR")){
    ...
    String formAction = "/rough/roughPurAction.do?method=save";
    ArrayList purIDnList = (ArrayList)request.getAttribute("purIdnList");
    if(purIDnList==null)
    purIDnList = new ArrayList();
 %>
 <tr>
  <td valign="top" class="hedPg">
...
<td><span class="txtLink" id="<%=fld_nme%>_TAB" style="<%=fld_typ%>" onclick="showHideDiv('.pktDtl','<%=fld_nme%>',this)"><%=fld_ttl%></span></td>
  ...
  int count =0;
   ...
   <%
           ...
           <% }%>
            <%if(lFld.equals("lot_dsc")){%>
                <img src="../images/genrate.png" onclick="LotIdnGenrate('<%=htmlFld%>')"/> 
            <%}} 
            if(fldTyp.equals("TA")) {
            %>
...
//                String lovQ = dao.getLOV_QRY(); //util.nvl((String)formFlds.get(lovKey));
 ...
 <%if(!method.equals("edit")){%>
  ...
  <html:submit property="reset" value="Reset" styleClass="submit"/>
   ...
   <div id="SEARCH" <%=search%>  class="pktDtl">
 ...
 <input type="hidden" name="nmeID" id="nmeID" value="">
  <div style="position: absolute;">
   ...
   <td>Type:</td>
   <td>
             <html:select property="value(Typ)" styleId="Typ" name="roughpurform" >
   ...
   <%pageList=(ArrayList)pageDtl.get("PKT_TYP");
           ...
           <td>Pkt Type:</td>
           <td>
             <html:select property="value(pkt_ty)" styleId="pkt_ty" name="roughpurform" >
             <html:option value="" >--select--</html:option>
             <!--<html:option value="B">Buyer</html:option>
             <html:option value="R">Review</html:option>-->
            <%for(int j=0;j<pageList.size();j++){
      ...
      <tr>
   <td>Purchase Idn</td>
   <td><html:textarea property="value(searchpur_idn)" name="roughpurform" styleId="searchpur_idn"/></td>
   </tr>
   <tr>
   <td colspan="2">
   <table>
 ...
 <%
    ...
    <%
       ...
       String lot_dsc= (String)nmeRel.getValue("lot_dsc");
        
      
       String onclick = "uploadPurfile("+lIdn+")";
        String edtLnk = "<a href=\""+ info.getReqUrl() + "/rough/roughPurAction.do?method=edit&purIdn="+lIdn+"\" >Edit</a>";
         String addLnk ="";
        pageList=(ArrayList)pageDtl.get("DIRECTENTY");
         if(pageList!=null && pageList.size() >0){
         for(int j=0;j<pageList.size();j++){
            pageDtlMap=(HashMap)pageList.get(j);
            dflt_val=(String)pageDtlMap.get("dflt_val");
       ...
       addLnk = "<a href=\""+ info.getReqUrl() + "/purchase/purchaseDtlAction.do?method=load&rughForm=Y&flg=M&purIdn="+lIdn+"&lotDsc="+lot_dsc+"\" >Add</a>";
         }else{
            addLnk = "<a href=\""+ info.getReqUrl() + "/rough/roughPurDtlAction.do?method=load&purIdn="+lIdn+"&lotDsc="+lot_dsc+"\" >Add</a>";
      ...
      String delLnk = "<a href=\""+ info.getReqUrl() + "/rough/roughPurAction.do?method=delete&purIdn="+lIdn+"\" >Del</a>";
        ...
        String link = info.getReqUrl() + "/rough/roughPurAction.do?method=delete&purIdn="+lIdn;
        String onclickdel = "return setDeleteConfirm('"+link+"')";
        delLnk = "<a href=\""+link +" \" onclick=\""+onclickdel+"  \">Del</a>";
              %>
        <td><%=lIdn%></td>
     <%   
        for(int j=0; j < daos.size(); j++) {
       ...
       <!--<%=edtLnk%>&nbsp;|&nbsp;<%=addLnk%>&nbsp;|&nbsp;<%=delLnk%>&nbsp;|&nbsp;<%=report%>-->
        <%pageList=(ArrayList)pageDtl.get("ACTION");
            ...
            if(fld_ttl.equals("Report")){%>
            |&nbsp;<%=report%>
            <%}else{
            if(lov_qry.equals("CHK") && purIDnList.contains(lIdn)){
             }else{ %>
            <%=dflt_val%>
            <%}}}}%>
