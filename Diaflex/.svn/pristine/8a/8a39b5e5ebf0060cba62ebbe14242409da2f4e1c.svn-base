...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
   ...
   <%
       ...
       <% }
        ...
        <div onclick="displayDiv('refDiv')"> <a href="<%=url%>"  target="_blank"><span class="txtLink" >Click Here for Report</span></a> </div></td>
            <%pageList= ((ArrayList)pageDtl.get("REPORT") == null)?new ArrayList():(ArrayList)pageDtl.get("REPORT");            
            if(pageList!=null && pageList.size() >0){
            for(int j=0;j<pageList.size();j++){
            pageDtlMap=(HashMap)pageList.get(j);
            ...
            val_cond=val_cond.replaceAll("~ACCESSID~",String.valueOf(accessidn));
            val_cond=val_cond.replaceAll("~URL~", info.getReqUrl());
            val_cond=val_cond.replaceAll("~SALID~", (String)request.getAttribute("saleId"));%>
            <td><a href="<%=val_cond%>"  target="_blank"><span class="txtLink" ><%=fld_ttl%></span></a></td>
            <%}}%>
        <%if(cnt.equals("xljf")){
       ...
       System.out.println(performaLink);
        ...
        String urlgenerate=info.getReqUrl()+"/marketing/performa.do?method=loadFromForm&idn="+request.getAttribute("saleId")+"&form=Y&typ=SL&pktTyp=SINGLE";
        %>
        <tr>
        <td height="15"  valign="top"><a href="<%=url%>"  target="_blank"><span class="txtLink" >Performa Invoice</span></a>&nbsp;&nbsp;
        <a href="<%=urlgenerate%>"  target="_blank"><span class="txtLink" >Generate Performa</span></a></td>
        </tr>
       <% }%>
        </table></td></tr>
  ...
  %>
    ...
    <html:form action="/marketing/memoSale.do?method=load" method="POST">
    <table><tr><td valign="top">
        <table class="grid1" >
        ...
        <tr><th colspan="2">Memo Search </th></tr>
           ...
           <tr><td>Packets. </td><td>
            <html:textarea property="value(vnmLst)" name="memoSaleForm" styleId="vnmLst" /><label id="fldCtr" ></label> </td> </tr>
            <tr><td colspan="2">
            <%
 ...
 %>
 RFID Device: <html:select property="value(dvcLst)" styleId="dvcLst" name="memoSaleForm"  >
...
<%}%>
    ...
    <tr> <td><b>Value </b></td> <td><bean:write property="vlu" name="memoSaleForm" /></td><td><span id="vlu">0</span>
    </td> </tr>
    ...
    <%
    if(view.equals("Y")) {
        String formAction = "/marketing/memoSale.do?method=save&memoIdn="+ reqIdn ; 
...
<tr><td><html:submit property="value(submit)" onclick="return confirmChangesWithPopup()"  value="Submit" styleClass="submit"/> </td> </tr>
        ...
        <html:hidden property="nmeIdn" name="memoSaleForm" styleId="nmeIdn"  />
          ...
          <td><b>Sale Person :</b></td><td> <bean:write property="value(SALEMP)" name="memoSaleForm"/> </td>
          <logic:notEqual property="value(shortTrms)"  name="memoSaleForm"  value="N" >
          <td><b>Terms  :</b></td><td> <bean:write property="value(shortTrms)" name="memoSaleForm"/> </td>
          </logic:notEqual>
          ...
          <logic:notEqual property="value(LOY_CTG)"  name="memoSaleForm"  value="NA" >
          <td><b>Loyalty :</b></td> <td> <bean:write property="value(LOY_CTG)" name="memoSaleForm"/> </td>
           </logic:notEqual>
           <logic:equal property="value(saleVlu)"  name="memoSaleForm"  value="Y" >
           <td>
            <b><bean:write property="value(saleVlubyr)" name="memoSaleForm" /></b>
           </td>
           </logic:equal>
           ...
           <td><b>Remark/Comment</b>&nbsp;<html:text property="value(rmk)" size="15"  /></td>
            ...
            <td>&nbsp;Note Person:&nbsp;</td>
            <td>
                  <html:select name="memoSaleForm" property="value(noteperson)" styleId="noteperson">
            ...
            <%}%>
             <logic:equal property="value(saleExYN)"  name="memoSaleForm"  value="Y" >
             <td>Employee</td>
             <td>
             <html:select property="value(empId)" name="memoSaleForm" styleId="empLst">
             <html:option value="" >--select--</html:option>
             <html:optionsCollection property="empLst" name="memoSaleForm"  value="byrIdn" label="byrNme" />
             </html:select>
             </td>
             </logic:equal>
          </tr>
       ...
       <logic:equal property="value(fnlexhRteDIS)"  name="memoSaleForm"  value="Y" >
           ...
           <td>
            <html:text property="value(fnlexhRte)" onchange="SetCalculationOnFnlExrt()"  styleId="fnlexhRte" size="5" name="memoSaleForm" />
           </td>
           ...
           <%pageList=(ArrayList)pageDtl.get("BROKER_SLAB");
            if(pageList!=null && pageList.size() >0){%>
            <td>Brokerage Slab:</td><td><html:select property="value(broker_slab)" styleId="broker_slab" name="memoSaleForm" >
           <html:option value="">---select---</html:option>
            <%for(int j=0;j<pageList.size();j++){
            pageDtlMap=(HashMap)pageList.get(j);
            dflt_val=(String)pageDtlMap.get("dflt_val"); 
          ...
          <tr><td>
            ...
            addrHiddval="N";
                ...
                <html:hidden property="value(addr_HIDD)" styleId="addr_HIDD" name="memoSaleForm" value="<%=addrHiddval%>"/>
               </td>
          ...
          <td>
           <html:select property="value(grpIdn)" name="memoSaleForm"  styleId="grpIdn"  >
            ...
            <% pageList= ((ArrayList)pageDtl.get("THRU_BANK") == null)?new ArrayList():(ArrayList)pageDtl.get("THRU_BANK");  
            if(pageList!=null && pageList.size() >0){
            for(int j=0;j<pageList.size();j++){
            pageDtlMap=(HashMap)pageList.get(j);
            dflt_val=util.nvl((String)pageDtlMap.get("dflt_val"));
...
<html:hidden property="value(brk4commval)" styleId="brk4commval" name="memoSaleForm" value="0"/>
            ...
            <td>&nbsp;No</td>
            <td><html:radio property="value(aadatpaid)"  onchange="setBrokerComm('aadatcomm','NN')" styleId="aadatpaid3" value="NN" name="memoSaleForm"/></td>
            <td>&nbsp;Net</td>
           ...
           <td id="brk1Nme"><bean:write property="value(brk1)" name="memoSaleForm" /></td>
           <td><html:radio property="value(brk1paid)" styleId="brk1paid1" value="Y" name="memoSaleForm"/></td>
           <td>&nbsp;Yes</td>  
           <td><html:radio property="value(brk1paid)"  styleId="brk1paid2"  value="N" name="memoSaleForm"/></td>
           <td>&nbsp;No</td> 
           <td><html:radio property="value(brk1paid)"  styleId="brk1paid3"  value="NN" name="memoSaleForm"/></td>
           <td>&nbsp;Net</td> 
           <td><html:text property="value(brk1comm)"  styleId="brk1comm"  readonly="true" size="3" name="memoSaleForm" /> </td>
           </tr>
           </table>
           </div>
           </td>
    ...
    <%}}}%>
            ...
            <%}}%>
                    ...
                    <th>Quot</th>
                    <th>Profit %</th>
                    <%pageList= ((ArrayList)pageDtl.get("MNLSALE_ID") == null)?new ArrayList():(ArrayList)pageDtl.get("MNLSALE_ID");     
                   if(pageList!=null && pageList.size() >0){
                   String chksrvTxtAll = "chksrvTXTAll('mnlsale_id')";%>
                    <th>Sale Id 
                    <html:text property="value(mnlsale_id)" styleId="mnlsale_id" name="memoSaleForm"  style="width:100px" onchange="<%=chksrvTxtAll%>" />
                    </th>
                    <%}%>
            ...
            itemHdr.add("RapRte");itemHdr.add("RapVlu");itemHdr.add("Dis");itemHdr.add("Prc / Crt");itemHdr.add("ByrDis");itemHdr.add("Quot");
             int count =0 ;
                ...
                <%   
                    ...
                    String mnlsaleidVal = "value(mnlsale_id_" + pktIdn + ")" ;
                    String mnlsaleid = "mnlsale_id_" + pktIdn;
                    ...
                    pktPrpMap.put("pktstt",pktstt);
                    pktPrpMap.put("QTY",util.nvl(pkt.getQty()));
                     priMap.put(String.valueOf(pktIdn), pkt.getMemoQuot());
             %>
                             <%
    pageList= ((ArrayList)pageDtl.get("RADIOBODY") == null)?new ArrayList():(ArrayList)pageDtl.get("RADIOBODY");                 
            ...
            <%}}%>
                 ...
                 <td nowrap="nowrap"><%=util.nvl(pkt.getValue("appdte"))%></td>
               ...
               if(pageList!=null && pageList.size() >0){%>
               <td>
               <html:text property="<%=mnlsaleidVal%>" styleId="<%=mnlsaleid%>" size="10" name="memoSaleForm" />
                </td>
                <%}%>
