...
<%@ page import="java.util.LinkedHashMap,java.util.Set,java.util.ArrayList,java.util.HashMap,java.sql.ResultSet,ft.com.*, java.util.Date, java.util.ArrayList, ft.com.dao.*"%>
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <%}%>
   ...
   <html:form action="mix/tranMixMktAction.do?method=fetch" onsubmit="return validate_trfToMkt();" method="post">
   <table  class="grid1">
  ...
  %>
  <img src="../images/edit.jpg" title="Edit Properties Module" onclick="newWindow('<%=info.getReqUrl()%>/dynamicProperty.do?method=load&mdl=TRF_MIX_SRCH&sname=TRF_MIX_SRCH&par=GENERIC')" border="0" width="15px" height="15px"/> 
  <%}%>
  </th></tr>
  <tr>
   <td colspan="2">
   ...
   if(stockListMap!=null && stockListMap.size()>0){
    ArrayList vwPrpLst = (ArrayList)session.getAttribute("TRF_MIX_VIEW");
   ...
   <html:form action="mix/tranMixMktAction.do?method=save" method="post" onsubmit="return validate_issue('CHK_' , 'count')">
      <html:hidden property="value(lstNme)" name="mixAssortIssueLKForm" value="<%=lstNme%>" />

   
   <tr><td valign="top" class="tdLayout">
   <table><tr><td>
   <html:submit property="value(issue)" styleClass="submit" value="Transfer" /></td> 
   <%
  ArrayList rolenmLst=(ArrayList)info.getRoleLst();
  String usrFlg=util.nvl((String)info.getUsrFlg());
  if(usrFlg.equals("SYS") || rolenmLst.contains("ADMIN")){
  %>
  <td><img src="../images/edit.jpg" title="Edit Properties Module" onclick="newWindow('<%=info.getReqUrl()%>/dynamicProperty.do?method=load&mdl=TRF_MIX_VIEW&sname=TRF_MIX_VIEW&par=A')" border="0" width="15px" height="15px"/> </td>
  <%}%>
  </tr></table>
  </td>
   </tr>
...
<td><%=stockPkt.getValue("qty")%></td>
<%for(int j=0; j < vwPrpLst.size(); j++ ){
    String lprp = (String)vwPrpLst.get(j);
    if(lprp.equals("CRTWT"))
      lprp="cts";
   %>
    <td><%=stockPkt.getValue(lprp)%>
    <%if(lprp.equals("RTE")){%>
    <input type="text"  name="<%=rteFldId%>" id="<%=rteFldId%>" value="<%=stockPkt.getValue(lprp)%>" />
    <%}%>
    </td>
     <%if(lprp.equals("RTE")){%>
      <td><%=stockPkt.getValue("AMT")%></td>
        <%}%>
<%}%>
</tr>
   <%}%>
