...
public class MixAssortReturnAction extends DispatchAction {
    
    public MixAssortReturnAction() {
        super();
    }
    public ActionForward load(ActionMapping am, ActionForm af, HttpServletRequest req,
      ...
      MixAssortReturnForm udf = (MixAssortReturnForm)af;
      udf.reset();
          GtMgrReset(req);
      ...
      String issIdnSql ="select to_char(sum(c.cts),'999990.00') cts , sum(c.qty) qty , trunc(((sum(trunc(c.cts,3)*nvl(c.upr, c.cmp)) / greatest(sum(trunc(c.cts,3)), 0.001)))) avg_Rte, b.flg1 , d.val , d.srt \n" + 
      "from iss_rtn a , iss_rtn_dtl b , mstk c , stk_dtl d , mprc e\n" + 
      "where a.iss_id = b.iss_id and b.iss_stk_idn=c.idn\n" + 
      "and b.stt='IS' and a.prc_id = ? and c.pkt_ty <> 'NR'\n" + 
      "and c.stt='MIX_AS_IS' and c.idn = d.mstk_idn and d.mprp='BOX_TYP' and d.grp=1\n" + 
      "and a.prc_id = e.idn and e.is_stt='MIX_AS_IS' \n" + 
      "group by b.flg1 , d.val , d.srt\n" + 
     ...
     ary.add(prcId);
      ...
      rs = (ResultSet)rsLst.get(1);
        while(rs.next()){
            ...
            mixIssueMap.put(box_typ+"_"+stt+"_CTS", cts);
            mixIssueMap.put(box_typ+"_"+stt+"_QTY", qty);
            mixIssueMap.put(box_typ+"_"+stt+"_AVG", avgRte);
            if(!boxTypLst.contains(box_typ))
          ...
          gtMgr.setValue(lstNme, mixIssueMap);
      ...
      MixAssortReturnForm udf = (MixAssortReturnForm)af;
          ...
          if(sttList!=null && sttList.size()>0){
                String sttStr = sttList.toString();
                 ...
                 sttCon = " and c.flg1 in ('"+sttStr+"')";
          }
       String box_typ = boxTypLst.toString();
          box_typ =box_typ.replaceAll(",", "','");
          box_typ =box_typ.replaceAll("\\[", "");
          box_typ =box_typ.replaceAll("\\]", "").replaceAll(" ", "");
       String iss_id =  "";
       double ttlMktCts =0;
          int ttlMktQty=0;
          ArrayList ary = new ArrayList();
         
       String ttlCtsSql  = "select  to_char(sum(a.cts),'999990.00') cts , sum(a.qty) qty,d.val boxid from mstk a , stk_dtl b , iss_rtn_dtl c  , stk_dtl d " + 
       "where a.idn=b.mstk_idn and b.mstk_idn=c.iss_stk_idn  " + 
       "and c.stt='IS' and a.stt='MIX_AS_IS' and b.mprp='BOX_TYP' and b.grp=1 "+sttCon+" and b.val in ('"+box_typ+"')"+
        " and d.mstk_idn=c.iss_stk_idn and d.mprp='BOX_ID'  and d.grp=1 group by d.val ";
        ArrayList outLst = db.execSqlLst("ttlCts", ttlCtsSql, ary);
        ...
        ResultSet rs = (ResultSet)outLst.get(1);
          while(rs.next()){
              ...
              String oldQtyTxt = "OLDQTY_"+boxId;
              oldQtyTxt = oldQtyTxt.replace("-", "_");
              udf.setValue(oldQtyTxt, qty);
          ...
          String prcId = "0";
          String wt_diff="0";
          String mprcIdn ="select idn ,wt_diff from mprc where is_stt='MIX_AS_IS'";
       outLst = db.execSqlLst("mprcInd", mprcIdn, new ArrayList());
         pst = (PreparedStatement)outLst.get(0);
        rs = (ResultSet)outLst.get(1);
          while(rs.next()){
              prcId = rs.getString("idn");
              wt_diff = util.nvl(rs.getString("wt_diff"),"0");
          }
          rs.close();
          pst.close();
        ...
        cst=null;
          String msg=null;
             ttlCtsSql  = "select  to_char(sum(a.cts),'999990.00') cts , sum(a.qty) qty,c.iss_id , b.val from mstk a , stk_dtl b , iss_rtn_dtl c  " + 
              "where a.idn=b.mstk_idn and b.mstk_idn=c.iss_stk_idn   "+sttCon+" " + 
              "and c.stt='IS' and a.stt='MIX_AS_IS' and b.mprp='BOX_TYP' and b.grp=1  and b.val in  ('"+box_typ+"') group by c.iss_id , b.val ";
              ary = new ArrayList();
             
           
                 HashMap boxIdBoxTypMap = new HashMap();
        outLst = db.execSqlLst("ttlCts", ttlCtsSql, ary);
          pst = (PreparedStatement)outLst.get(0);
         rs = (ResultSet)outLst.get(1);
              double ttlCts =0;
                 while(rs.next()){
          ...
          String ttlSql = 
          "select  to_char(sum(a.cts),'999990.00') cts , sum(a.qty) qty , trunc(((sum(trunc(a.cts,2)*nvl(a.upr, a.cmp)) / greatest(sum(trunc(a.cts,3)), 0.001)))) avg_Rte from mstk a , stk_dtl b , iss_rtn_dtl c \n" + 
          "where a.idn=b.mstk_idn and b.mstk_idn=c.iss_stk_idn  "+sttCon+" " + 
          "and c.stt='IS' and a.stt='MIX_AS_IS' and b.mprp='BOX_TYP' and b.grp=1  and b.val in ('"+box_typ+"') and c.iss_id = ? ";
          params = new ArrayList();
          params.add(String.valueOf(issueIdn));
        outLst = db.execSqlLst("ttlSql", ttlSql, params);
          pst = (PreparedStatement)outLst.get(0);
         rs = (ResultSet)outLst.get(1);
          while(rs.next()){
          ...
          req.setAttribute("WT_DIFF", wt_diff);
       ...
       init(req,res,session,util);
        ...
        try {
            if (asViewPrp == null) {
              ...
              ArrayList outLst = db.execSqlLst(" Vw Lst ", "Select prp  from rep_prp where mdl = 'BOX_VIEW' and flg='Y' order by rnk ",
                               new ArrayList());
              PreparedStatement pst = (PreparedStatement)outLst.get(0);
