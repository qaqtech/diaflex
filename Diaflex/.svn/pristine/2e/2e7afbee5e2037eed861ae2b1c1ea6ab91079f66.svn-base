...
<html>
  <head>
    ...
    <title>Name Detail Master</title>
...
<%}%>
  ...
  <div> <label class="redLabel"> <%=errors.get(i)%></label></div>
   <%}%>
   </td>
  <%}%>
    <tr><td class="tdLayout" valign="top">
 <html:form action="masters/atrDtl.do?method=save" method="post">
    <table class="grid1">
    ...
    for(int dsp=0; dsp < dspOrder.size(); dsp++) {
        ...
        for(int i=0; i < lmt; i++) {
            ...
            if(!(msg.equalsIgnoreCase("NEW"))) {
             lDao = (GenDAO)reqList.get(i);
              lIdn = lDao.getIdn();
              fldId = lIdn;
              dataTyp=util.nvl((String) lDao.getValue("dataTyp"));
            }
      ...
      <%
       ...
       if (fldId.indexOf("&") > -1) {
            ...
            String link1="/masters/atrDtlSub.do?method=load&mprp="+ fldId ;
       %>     
        <html:link page="<%=link1%>">Add Sub Details </html:link>
        <%}%>
        </td>
       <% }%>   
