...
<html>
 ...
 <body onfocus="<%=onfocus%>">
   <html:form action="purchase/purchaseDtlAction.do?method=addPrp" method="post">
    <html:hidden property="value(purIdn)" name="purchaseDtlForm" />
     <html:hidden property="value(purDtlIdn)" name="purchaseDtlForm"/>
  <table>
  <tr><td>
 ...
 <% if(typ.equals("C")){
  ...
  %>
  <html:select property="<%=fldName%>" name="purchaseDtlForm" >
  ...
  <%}else if(typ.equals("T")){%>
   <html:text property="<%=fldName%>" size="10" name="purchaseDtlForm"/>
  <%}else{%>
  <html:text property="<%=fldName%>" size="5" name="purchaseDtlForm"/>
  <%}%>
