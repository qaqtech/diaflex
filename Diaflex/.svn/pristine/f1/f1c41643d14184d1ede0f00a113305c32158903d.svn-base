...
<input type="hidden" id="clr" value="<%=dbinfo.get("CLR")%>" />
 <html:form action="/report/genericReport.do?method=save" method="post" target="" styleId="genFrm" onsubmit="return validateAnalysisRpt();"> 
 <%
 ...
 <% for(int n=0;n<sttList.size();n++){
  ...
  String chkid="STT_"+n;
  %>
  <!--<td><html:radio property="value(stt)" name="genericReportForm"   value="<%=grp1%>" /> </td><td><%=dsc%></td>-->
  <td><html:checkbox property="<%=dscgrp1%>" styleId="<%=chkid%>" value="<%=grp1%>" name="genericReportForm"/></td><td><%=dsc%></td>
  <%}%> <!--<%
      ...
      %>
      <td><html:text property="<%=fld_nme%>" size="2" name="genericReportForm" value="<%=dflt_val%>"/></td>
    <%}}
    %>
   <td>DAYS : <html:text property="value(DAYS)" size="2" name="genericReportForm"/></td>-->
  <td><table>
 ...
 <html:radio property="value(srchBy)"  styleId="bypacket" value="P" /> Packet.-->
  <%pageList=(ArrayList)pageDtl.get("SRCHBY");
   ...
   <table cellpadding="0" cellspacing="0">
       <tr>
         <td> <html:checkbox property="value(is3Ex)" value="is3Ex"   styleId="is3Ex" name="genericReportForm" onclick="setAnalysEX('EX',this)" />3 EX</td>
         <td> <html:checkbox property="value(is3VG)" value="is3VG"   styleId="is3VG" name="genericReportForm" onclick="setAnalysEX('VG',this)" />3 VG</td>
       </tr>
   </table>
  <%}%>
  </td></tr>
  ...
  if(btnKey!=null){%>
     Upgrade Type : <html:radio property="value(upgrade)" name="genericReportForm"   value="COL" />&nbsp;Color&nbsp;
   &nbsp;&nbsp;<html:radio property="value(upgrade)" name="genericReportForm"   value="CLR" />&nbsp;Clarity&nbsp;
   &nbsp;&nbsp;<html:radio property="value(upgrade)" name="genericReportForm"   value="BOTH" />&nbsp;Both&nbsp;
...
<%}    
            ...
            dflt_val=(String)pageDtlMap.get("dflt_val");
            val_cond="settarget('genFrm','fixed'); ";
            if(fld_nme.indexOf("hitmap") > -1)
            val_cond="settarget('genFrm','hitmap1'); ";
...
<tr>
<td>Ignore Memo</td>
<td><html:textarea property="value(ignoreememo)" name="genericReportForm" cols="20" rows="2" styleId="ignoreememo"/></td>
</tr>
<tr>
<td>Count Mfg_pri For Given Memo</td>
<td><html:textarea property="value(mfgprimemo)" name="genericReportForm" cols="20" rows="2" styleId="mfgprimemo"/></td>
</tr>
</table> </td> </tr>
<tr><td>
...
<tr>
<td>Sold Date : </td>  
<td><html:text property="value(frmDtestkoenclose)" styleId="frmDtestkoenclose" name="genericReportForm" size="10" /><img src="<%=info.getReqUrl()%>/images/calender.png" onClick="setYears(2011, <%=info.getCurrentYear()%>);showCalender(this, 'frmDtestkoenclose');"> 
To&nbsp; <html:text property="value(toDtestkoenclose)" styleId="toDtestkoenclose" name="genericReportForm"  size="10"/> <img src="<%=info.getReqUrl()%>/images/calender.png" onClick="setYears(2011, <%=info.getCurrentYear()%>);showCalender(this, 'toDtestkoenclose');"></td>
</tr>
<tr>
<td>Date : </td>  
<td><html:text property="value(frmdtestkoenclose)" styleId="frmdtestkoenclose" name="genericReportForm" size="10" /><img src="<%=info.getReqUrl()%>/images/calender.png" onClick="setYears(2011, <%=info.getCurrentYear()%>);showCalender(this, 'frmdtestkoenclose');"> 
...
<td>
      ...
      if(pageList!=null && pageList.size() >0){%>
      <html:select  property="value(level1openclose)" name="genericReportForm" styleId="level1openclose" style="width:100px"   > 
      <%for(int j=0;j<pageList.size();j++){
      pageDtlMap=(HashMap)pageList.get(j);
      fld_ttl=(String)pageDtlMap.get("fld_ttl");
...
<div id="popupContactNewGoods" align="center" >
<table align="center" cellpadding="10" cellspacing="10">
<tr>
<td>Date : </td>  
<td><html:text property="value(fromnewgoods)" styleId="fromnewgoods" name="genericReportForm" size="10" /><img src="<%=info.getReqUrl()%>/images/calender.png" onClick="setYears(2011, <%=info.getCurrentYear()%>);showCalender(this, 'fromnewgoods');"> 
To&nbsp; <html:text property="value(tonewgoods)" styleId="tonewgoods" name="genericReportForm"  size="10"/> <img src="<%=info.getReqUrl()%>/images/calender.png" onClick="setYears(2011, <%=info.getCurrentYear()%>);showCalender(this, 'tonewgoods');"></td>
</tr>
<tr><td colspan="3" align="center"><table><tr>
<td><html:submit property="value(newgoodsgrid)" value="Search" onclick="disablePopupNewGoods()" styleClass="submit" /> </td>
<td><button type="button" onclick="disablePopupNewGoods()" Class="submit" >Back</button> </td>
</tr></table></td>
</tr>
</table>
</div>
  ...
  %>
   ...
   %>
    ...
    <table cellpadding="3" width="100px"  cellspacing="2" class="bground">
           ...
           <html:select  property="<%=prpFld1%>" name="genericReportForm"  style="width:75px" onchange="<%=onChg1%>" styleId="<%=fld1%>"  > 
           ...
           <html:select property="<%=prpFld2%>" name="genericReportForm"  style="width:75px" onchange="<%=onChg2%>" styleId="<%=fld2%>"  > 
...
//                   }
             %>
             <tr><td align="center"><html:checkbox  property="<%=chFldNme%>"  name="genericReportForm" styleId="<%=checId%>"  onclick="<%=onclick%>" value="<%=vall%>"/></td><td align="center"><%=pPrtl%></td></tr>
             <%}%>
             </table></div>
     ...
     <tr>
      <%if(lprpTyp.equals("D")){%>
        ...
        <table><tr>
        <td><html:text property="<%=prpFld1%>"  styleClass="txtStyle"  name="genericReportForm" styleId="<%=fld1%>"   size="9" maxlength="25" /> </td>
        ...
        <td><html:text property="<%=prpFld2%>"  styleClass="txtStyle"  name="genericReportForm"   size="9" maxlength="25" /> </td>
        ...
        <html:text property="<%=prpFld1%>"  styleClass="txtStyle"  name="genericReportForm" styleId="<%=fld1%>"  /> 
        </td>
       
        <%} else{%>
       
           <td bgcolor="#FFFFFF" align="center"><html:text property="<%=prpFld1%>"  styleClass="txtStyle"  name="genericReportForm" styleId="<%=fld1%>"  size="11" maxlength="25" onchange="isNumericDecimal(this.id)" /></td>
        <td bgcolor="#FFFFFF" align="center"><html:text property="<%=prpFld2%>"   styleClass="txtStyle" name="genericReportForm"   size="11" maxlength="25" onchange="isNumericDecimal(this.id)" /></td>
 ...
 <table>
 <tr>
<td colspan="2"><html:textarea property="value(vnm)" name="genericReportForm" cols="30" rows="15" styleId="pid"/></td>
</tr>
 </table>
 </td></tr>
              </table>
  ...
  <table><tr><td>Search Type:</td><td align="left">
      ...
      if(pageList!=null && pageList.size() >0){%>
      <html:select  property="value(srchTyp)" name="genericReportForm" onchange="GenericSrchDW(this)" styleId="srchTyp" style="width:100px"   > 
    ...
    <%}%>
              ...
              <td>Id </td><td>
              <div>
              <html:text property="value(idn)"  name="genericReportForm"   />
              </div>
              </td></tr></table>
              </div></td></tr>
               <tr>
...
if(pageList!=null && pageList.size() >0){%>
<html:select property="value(compareWith)" name="genericReportForm" styleId="compareWith" style="width:100px" >
<%for(int j=0;j<pageList.size();j++){
pageDtlMap=(HashMap)pageList.get(j);
fld_ttl=(String)pageDtlMap.get("fld_ttl");
dflt_val=(String)pageDtlMap.get("dflt_val"); %>
...
<%}%>
 ...
 <tr><td>
              ...
              <td>
              <html:select  property="value(period)" name="genericReportForm" styleId="period" > 
  ...
  <tr><td align="left">Grid Format:
    <html:select  property="value(gridfmt)" name="genericReportForm" styleId="gridfmt" > 
...
if(pageList!=null && pageList.size() >0){%>
<td align="left">Grid By:
<html:select  property="value(gridby)" name="genericReportForm" styleId="gridby" > 
<%for(int j=0;j<pageList.size();j++){
pageDtlMap=(HashMap)pageList.get(j);
fld_ttl=(String)pageDtlMap.get("fld_ttl");
fld_nme=(String)pageDtlMap.get("fld_nme"); %>
...
<%}%>    
  ...
  <%}
  count++;%>
  <td><html:checkbox  property="<%=chFldNme%>"  name="genericReportForm" styleId="<%=checId%>"  onclick="" value="<%=mprcId%>"/> <%=util.nvl((String)mprcBeanAll.get(mprcId))%></td>
  <%}%>
  </tr>
  <tr>
  <td>Process Status</td>
  <td>
  <html:select property="value(prcstt)" styleId="prcstt" name="genericReportForm" >
  ...
  <td>Type</td>
  <td>
  <html:select property="value(rpttype)" styleId="rpttype" name="genericReportForm" >
  ...
  <td>Group <%=p%></td>
  <td>
  <html:select property="<%=chFldNme%>" styleId="<%=checId%>" name="genericReportForm" >
  ...
  <td>Report</td>
  <td>
  <html:select property="value(reportNme)" styleId="reportNme" name="genericReportForm" >
