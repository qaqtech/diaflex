...
<html:form action="/box/boxCrt.do?method=save" method="post" >
<html:hidden property="value(oldCrtId)" name="boxCriteriaForm" />
<input type="hidden" value="" name="subprpval" id="subprpval">

<%
// srchPrpList contains all properties to be shown in the Jsp
...
ArrayList typeList =((ArrayList)util.getFromMem(info.getMem_ip(),info.getMem_port(),info.getCnt()+"_typeList") == null)?new ArrayList():(ArrayList)util.getFromMem(info.getMem_ip(),info.getMem_port(),info.getCnt()+"_typeList"); 
...
<tr><td><table>
<tr>
<td colspan="3">Existing Criteria</td>
<td>Type</td>
<td>Description</td>
<td>Property</td>
...
<tr>
<!-- Existing Criteria -->
<td><html:select property="value(exiCrt)" name="boxCriteriaForm" style="width:125px" onchange="selectCrt();" styleId="exiCrt" >
...
<td valign="top">
<html:submit property="value(loadFav)" value="Load" styleClass="submit" /></td>
<td valign="top">
<html:submit property="value(removeCrt)" value="Remove" styleClass="submit" /></td>
<!-- type -->
<td><html:select property="value(type)" name="boxCriteriaForm" style="width:125px" onchange="" styleId="type" >
...
<td>
<html:select property="value(prp)" name="boxCriteriaForm" style="width:125px" onchange="dspSubProp(this);" styleId="prp" >
...
{
%>
<td> <html:select property="value(subPrp)" name="boxCriteriaForm" style="" styleId="subPrp" >
...
if(prpValVec!=null && prpValVec.size()>0){
...
{
%>
<td> <html:select property="value(subPrp)" name="boxCriteriaForm" style="display:none" styleId="subPrp" >
</html:select></td>
<% }
%>
...
<td valign="top">
<html:submit property="value(updateCrt)" value="Update" styleClass="submit" />
<%
  ArrayList rolenmLst=(ArrayList)info.getRoleLst();
  String usrFlg=util.nvl((String)info.getUsrFlg());
  if(usrFlg.equals("SYS") || rolenmLst.contains("ADMIN")){
  %>
  &nbsp;&nbsp;<img src="../images/edit.jpg" title="Edit Properties Module" onclick="newWindow('<%=info.getReqUrl()%>/dynamicProperty.do?method=load&mdl=MEMO_CRT&sname=stkCrtprplist&par=STK_CRT')" border="0" width="15px" height="15px"/> 
  <%}%>
  </td>
</tr>
</table></td></tr>
...
<tr><td>Carat</td>
<td ><html:text property="value(wt_fr_c)" size="6" styleId="wt_fr_c" onchange="<%=txtBoxOnChg1%>" styleClass="txtCWTWt" name="boxCriteriaForm" /></td>
<td ><html:text property="value(wt_to_c)" size="6" styleId="wt_to_c" onchange="<%=txtBoxOnChg2%>" styleClass="txtCWTWt" name="boxCriteriaForm"/></td>
</tr>
<%
...
%>
<tr><td><html:checkbox property="<%=chkFldId%>" name="boxCriteriaForm" styleId="<%=checkId%>" value="<%=valWt%>" onclick="<%=onclick%>" ></html:checkbox> &nbsp;<%=valWt%></td>
...
{
%>
<html:text property="<%=mutiText%>" name="boxCriteriaForm" size="30" styleId="<%=mutiTextId%>" styleClass="txtStyle" />
<% }
else
{
%>
<html:text property="<%=mutiText%>" value="<%=loadStr%>" name="boxCriteriaForm" size="30" styleId="<%=mutiTextId%>" styleClass="txtStyle" />
<%
}
%>
</td>
...
<html:select property="<%=prpFld1%>" name="boxCriteriaForm" style="width:75px" onchange="<%=onChg1%>" styleId="<%=fld1%>" >
...
<html:select property="<%=prpFld2%>" name="boxCriteriaForm" style="width:75px" onchange="<%=onChg2%>" styleId="<%=fld2%>" >
...
// }
%>
<tr ><td align="center"><html:checkbox property="<%=chFldNme%>" name="boxCriteriaForm" styleId="<%=checId%>" onclick="<%=onclick%>" value="<%=vall%>"/></td><td align="center"><%=pPrtl%></td></tr>
<%}%>
</table>
...
{
%>
<html:text property="<%=mutiText%>" value="<%=loadStrL%>" name="boxCriteriaForm" size="30" styleId="<%=mutiTextId%>" styleClass="txtStyle" />
<%
}
%>
</td>
...
%>
<td align="center">
<html:select property="<%=prpFld1%>" name="boxCriteriaForm" style="width:100px" onchange="<%=onChg1%>" styleId="<%=fld1%>" >
...
<html:select property="<%=prpFld2%>" name="boxCriteriaForm" style="width:100px" onchange="<%=onChg2%>" styleId="<%=fld2%>" >
...
<html:text property="<%=prpFld1%>" name="boxCriteriaForm" styleId="<%=fld1%>" onkeypress="focout();" value="<%=val1%>" size="9" maxlength="25" /> <a href="#" onClick="setYears(1900, '<%=info.getCurrentYear()%>' );showCalender(this, '<%=prpFld1%>');">
<img src="<%=info.getReqUrl()%>/images/calender.png"></a></td>
<td bgcolor="#FFFFFF" align="center"><html:text property="<%=prpFld2%>" styleId="<%=fld2%>" name="boxCriteriaForm" value="<%=val2%>" size="9" maxlength="25" /> <a href="#" onClick="setYears(1900, '<%=info.getCurrentYear()%>' );showCalender(this, '<%=prpFld2%>');">
<img src="<%=info.getReqUrl()%>/images/calender.png"></a></td>

<%} else if(lprpTyp.equals("T")){%>
<td bgcolor="#FFFFFF" align="center" colspan="2">
<html:text property="<%=prpFld1%>" name="boxCriteriaForm" styleId="<%=fld1%>" value="<%=val1%>" size="30" maxlength="20" />
</td>

<%} else{%>
<td bgcolor="#FFFFFF" align="center"><html:text property="<%=prpFld1%>" name="boxCriteriaForm" styleId="<%=fld1%>" size="11" maxlength="25" onchange="isNumericDecimal(this.id)" /></td>
<td bgcolor="#FFFFFF" align="center"><html:text property="<%=prpFld2%>" name="boxCriteriaForm" size="11" maxlength="25" onchange="isNumericDecimal(this.id)" /></td>
<%}}
