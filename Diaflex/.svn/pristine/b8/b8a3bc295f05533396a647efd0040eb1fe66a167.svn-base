...
public class ReportModuleAction extends DispatchAction {

    private final String formName   = "reportmoduleform";


        public ReportModuleAction() {
        super();
    }
    ...
    public ActionForward save(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
        ...
        if(!rtnPg.equals("sucess")){
            ...
            util.updAccessLog(req,res,"Report Module form", "save start");
        ...
        ReportModuleForm udf = (ReportModuleForm) af;
        ...
        if (udf.getAddnew() != null) {
            ...
            for (int i = 1; i <= lmt; i++) {
                ...
                String insIntoQ = " insert into df_rep_mdl ( idn ";
                String insValQ  = "values(df_rep_mdl_seq.nextval ";
        ...
        if (udf.getModify() != null) {
            ...
            String    getMenu = "select " + helper.getSrchFields(daos, "mdl", false) + " from df_rep_mdl  ";
            ArrayList    params  = new ArrayList();
            ResultSet rs      = db.execSql("getUIFormFields for update", getMenu, params);
          ResultSet rs1=null;
            while (rs.next()) {
                String    lIdn  = rs.getString(1);
                String updQ  = " update df_rep_mdl set idn = idn ";
                ...
                if (isChecked.length() > 0) {
                    ...
                    if (params.size() > 0 && errors.size()==0) {
                      String iquery="select idn from df_rep_mdl where mdl='"+lIdn+"'";
                      rs1 = db.execSql("iquery",iquery,  new ArrayList());
                      if(rs1.next()){
        ...
        req.setAttribute("errors", errorList);
            util.updAccessLog(req,res,"Report Module form", "save end");
    ...
    public ActionForward load(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
        ...
        if(!rtnPg.equals("sucess")){
            ...
            util.updAccessLog(req,res,"Report Module form", "load start");
            HashMap  formFields = info.getFormFields(formName);
        ...
        String MaxSrt="select max(srt) srt from df_rep_mdl";
        rs = db.execSql("maxsrt",MaxSrt,  new ArrayList());
        if(rs.next()){
          maxsrt=util.nvl(rs.getString(1));
        }
        req.setAttribute("maxsrt", maxsrt);
        // udf = (ReportModuleForm)helper.getForm(uiForms, "", srchFields, srchQ, new ArrayList(), daos, "VIEW") ;
        req.setAttribute(formName, list);
        rs.close();
        // udf.setList(list);
        // info.setNmeSrchList(list);
        util.updAccessLog(req,res,"Report Module form", "load end");
    ...
    public String init(HttpServletRequest req , HttpServletResponse res,HttpSession session,DBUtil util) {
            ...
            System.out.print("New Session Id :="+session.getId());
                ...
                util.updAccessLog(req,res,"Report Module form", "Unauthorized Access");
                else
                util.updAccessLog(req,res,"Report Module form", "init");
            }
            }
            return rtnPg;
         }
