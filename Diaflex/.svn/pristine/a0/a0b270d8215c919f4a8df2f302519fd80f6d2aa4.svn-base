...
<html>
  <head>
    ...
    <title>Process Property Setting</title>
...
DBUtil dbutil = new DBUtil();
    ...
    String keyNme = (String)info.getValue("PRC");
    if(pgTtl.indexOf("~") > -1)
      pgTtl = pgTtl.replaceFirst("~process", keyNme);
      
    dbutil.SOP(" View "+ formName + " : fields = " + daos.size());
    int addRec = Integer.parseInt(util.nvl(uiForms.getREC(),"10"));
  
    String formAction = "/masters/processPrpAction.do?method=save";
 %>
 <tr>
 ...
 <html:form action="<%=formAction%>" method="post">
  <html:hidden property="value(prcIdn)" />
    <table class="grid1">
...
<%
    ...
    for(int dsp=0; dsp < dspOrder.size(); dsp++) {
      ...
      <%
        ...
        <td>&nbsp;</td>
        <%} else {
            String link1 = info.getReqUrl() + "/masters/processPrpAction.do?method=delete&Idn="+fldId ;
