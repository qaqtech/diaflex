...
<script language="javaScript" type="text/javascript" src="<%=info.getReqUrl()%>/scripts/bse.js?v=<%=info.getJsVersion()%>"></script>
 <script language="javaScript" type="text/javascript" src="<%=info.getReqUrl()%>/jquery/jquery.freezeheader.js?v=<%=info.getJsVersion()%>"></script>
  ...
  <title>Pending For Delivery</title>
...
HashMap saleDtl=(HashMap)request.getAttribute("saleDtl");
HashMap saleTotalsDtl=(HashMap)request.getAttribute("saleTotalsDtl");
HashMap byrDtl=(HashMap)request.getAttribute("byrDtl");
ArrayList empList=(ArrayList)request.getAttribute("empList");
ArrayList saletyp=(ArrayList)request.getAttribute("saletyp");
...
String cnt = util.nvl((String)dbinfo.get("CNT"));
 if(pgtyp.equals(""))
     pgtyp=util.nvl((String)session.getAttribute("saletyp"));
 String emp="";
 String byr="";
 String copybyr="";
          ...
          %>
             <%
              ...
              if(byr!=null && !byr.equals("") && !byr.equals(copybyr)){
                  ...
                  <%if(!pendf.equals("PNDF_MEMO")){%>
                  <td align="center"><img src="../images/terms.png" title="Get Memo Terms Wise" onclick="callpendfSalesametermsPkt('<%=byr_idn.get(emp+"_"+byr)%>','<%=loop%>','S',this)" border="0" width="15px" height="15px"/> </td>
                  <%}%>
                  <% for(int k=0;k<saletyp.size();k++){
                        ...
                        if(dtl!=null && dtl.size()> 0){
                            ...
                            %>
                            <td align="right"><a onclick="callpendfSalePkt('<%=byr_idn.get(emp+"_"+byr)%>','<%=loop%>','<%=saletyp.get(k)%>',this)" title="Click Here To See Details" style="text-decoration:underline"><%=dtl.get(0)%></a> </td>
