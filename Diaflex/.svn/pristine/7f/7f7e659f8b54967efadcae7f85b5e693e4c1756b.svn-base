...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <img src="../images/line.jpg" border="0" /></td><td valign="middle"> <span class="pgHed">Mixing Issue</span> </td>
   ...
   <table><tr><td>
   <html:form action="mix/mixissAction.do?method=fetch" method="post" onsubmit="return validate_memo()">
  <table  class="grid1">
  <tr><th> </th><th>Generic Search </th></tr>
  <tr><td valign="top">
   <table>
   <tr><td>Process</td><td>
    <html:select property="value(mprcIdn)"  styleId="mprcIdn"  name="mixissForm" disabled="<%=disabled%>"   >
         
            <html:optionsCollection property="mprcList"  name="mixissForm"
    ...
    Employee : </td>
   <td>
   <html:select property="value(empIdn)"  styleId="empIdn" name="mixissForm" disabled="<%=disabled%>"  >
           <html:option value="0" >--select--</html:option>
            <html:optionsCollection property="empList" name="mixissForm" 
            label="emp_nme" value="emp_idn" />
            </html:select>
   </td>
   </tr>
   <tr>
   <td>Packet No:</td><td> <html:textarea property="value(vnmLst)" rows="7"  cols="30" name="mixissForm"  /></td>
   </tr>
  </table></td>
   <td valign="top">
   ...
   <html:form action="mix/mixissAction.do?method=Issue" method="post" onsubmit="return validate_issue('CHK_' , 'count')">
   <html:hidden property="value(empId)" name="mixissForm" />
   <html:hidden property="value(prcId)" name="mixissForm" />
 ...
 String checkFldVal = "value("+memo+"_"+sh+"_"+size+")";
 %>
<tr>
<td><html:checkbox property="<%=checkFldVal%>" styleId="<%=checkFldId%>" name="mixissForm" onclick="<%=onclick%>" value="yes" /> </td>
<td><%=sr%></td>
<td><%=stockPkt.get("qty")%></td>
<td><%=cts%></td><td><%=memo%></td><td><%=sh%></td><td><%=size%></td>
</tr>
   <%}%>
   </table></td></tr>
