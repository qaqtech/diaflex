...
<html>
  <head>
    ...
    <title>Diaflex :NameDetails</title>
...
<%
 ...
 <%
  ...
  <html:form action="contact/nmedtl.do?method=save" method="post">
      ...
      <%
        for(int j=0; j < daos.size(); j++) {
            ...
            if(valCond.indexOf("~fldId") > -1)
                valCond = valCond.replaceAll("~fldId", htmlFld);
        ...
        <%
            if(fldTyp.equals("T")) {
            %>
                <html:text property="<%=fldNm%>" styleId="<%=htmlFld%>" size="<%=fldSz%>" onchange="<%=valCond%>"/>
            <%}
       ...
       <% if(msg.equalsIgnoreCase("NEW")) {%>
        ...
        <%}else {
          ...
          String link= url+"/contact/nmedtl.do?method=delete&nme_dtl_idn="+fldId ;
          String onclick = "return setaddDeleteConfirm('"+link+"')";
