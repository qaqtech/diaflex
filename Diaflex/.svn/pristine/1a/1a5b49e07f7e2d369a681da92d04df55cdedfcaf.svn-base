...
public class DailyApproveReportAction extends DispatchAction {
    public ActionForward load(ActionMapping am, ActionForm form,
        ...
        DailyApproveReportForm dailyApproveReportForm = (DailyApproveReportForm)form;
      ResultSet rs = null;
      ...
      String dteto = " trunc(sysdate) ";
        ...
        if(flg.equals("New")){
            dailyApproveReportForm.reset();
        }
      String dfr = util.nvl((String)dailyApproveReportForm.getValue("dtefr"));
      String dto = util.nvl((String)dailyApproveReportForm.getValue("dteto"));
      String spersonId = util.nvl((String)dailyApproveReportForm.getValue("styp"));
      String typ = util.nvl((String)dailyApproveReportForm.getValue("typ"));
      String buyerId = util.nvl(req.getParameter("nmeID"));
      String group = util.nvl((String)dailyApproveReportForm.getValue("group"));
      String memofr = util.nvl((String)dailyApproveReportForm.getValue("memofr"));
      String memoto = util.nvl((String)dailyApproveReportForm.getValue("memoto"));
      ...
      if(!spersonId.equals("") && spersonId.length()>1){
        ...
        if(!typ.equals("") && typ.length()>1){
          ...
          sql =sql+ " and b.typ in ('IAP', 'EAP','WAP','LAP','MAP','SAP','HAP','BAP','KAP','BIDAP','OAP') ";  
        }
      ...
      if(!buyerId.equals("")){
       ...
       "  and b.stt not in ('APRT','RT','CL','CS','ALC','MRG') "+sql+""+bql+dteQ+
...
//  Added by Reshmi on 15-11-2011
       ...
       " where a.idn=b.idn and b.mstk_idn=c.idn  and c.pkt_ty in('NR','SMX') and nvl(c.prt2,'NA')=decode(nvl(c.prt2,'NA'),'SPECIAL',nvl(PACK_VAR.Get_Special,'NA'),nvl(c.prt2,'NA')) "+
        "  and b.stt not in ('APRT','RT','CL','CS','ALC','MRG')  "+sql+""+bql+
      ...
      String grandQry = " select sum(b.qty) qty "+
       ...
       " from mjan a,jandtl b, mstk c,mnme d "+
       " where a.idn=b.idn and b.mstk_idn=c.idn  and c.pkt_ty in('NR','SMX') and nvl(c.prt2,'NA')=decode(nvl(c.prt2,'NA'),'SPECIAL',nvl(PACK_VAR.Get_Special,'NA'),nvl(c.prt2,'NA'))"+
      "  and b.stt not in ('APRT','RT','CS','CL','ALC','MRG')  "+sql+""+bql+
      " and d.nme_idn=a.nme_idn "+dteQ;
      ...
      while (rs.next()) {
        ...
        dailyApproveReportForm.setValue("memotbl", memotbl);
        HashMap allPageDtl= (info.getPageDetails() == null)?new HashMap():(HashMap)info.getPageDetails();
        HashMap pageDtl=(HashMap)allPageDtl.get("DAILY_APPROVE_RPT");
        if(pageDtl==null || pageDtl.size()==0){
        pageDtl=new HashMap();
        pageDtl=util.pagedef("DAILY_APPROVE_RPT");
        allPageDtl.put("DAILY_APPROVE_RPT",pageDtl);
        }
    ...
    public ActionForward pktDtlExcel(ActionMapping am, ActionForm form,
       ...
       String dteto = " trunc(sysdate) ";
        ...
        " ( Select c.sk1,a.idn approve_id,nvl(nvl(nvl(st.txt,st.dte),st.num),st.prt1) atr , mprp,decode(a.flg, 'NN','DF', 'CNF', 'WEB', 'DF') frm,c.idn,to_char(b.cts,'99999999990.99') cts,c.vnm,c.rap_rte raprte,nvl(c.upr,c.cmp) rate, byr.get_nm(nvl(d.emp_idn,0)) sale_name, DECODE(upper(NVL(pack_var.get_usr, 'NA')), 'DEMO', byr.get_nm(nvl(d.nme_idn,0)),d.fnme) byr,nvl(b.fnl_sal, b.quot) memoQuot\n" + 
            ...
            "and b.stt not in ('APRT','RT','CS','CL','ALC','MRG') and b.typ in ('IAP','OAP', 'EAP','WAP','LAP','MAP','SAP','HAP','BAP','KAP','BIDAP') And d.Nme_Idn= ?\n";
     ...
     String dteto = " trunc(sysdate) ";
      ...
      if(!nmeidn.equals("") && !dte.equals("")){
       ...
       String pktDtlSql =
        ...
        " and b.stt not in ('APRT','RT','CS','CL','ALC','MRG') and b.typ in ('IAP','OAP', 'EAP','WAP','LAP','MAP','SAP','HAP','BAP','KAP','BIDAP') and a.trms_idn=e.idn And d.Nme_Idn= ? \n";
  ...
  public ActionForward pktDtlExcelXLFNL(ActionMapping am, ActionForm form,
      ...
      if(!nmeidn.equals("") && !dte.equals("")){
        ...
        " from mjan a,jandtl b, mstk c,mnme d , mtrms e where "+
        "a.idn=b.idn and b.mstk_idn=c.idn and d.nme_idn=a.nme_idn  and c.pkt_ty in('NR','SMX')  "+
        " and b.stt not in ('APRT','RT','CS','CL','ALC','MRG') and b.typ in ('IAP','OAP', 'EAP','WAP','LAP','MAP','SAP','HAP','BAP','KAP','BIDAP') and a.trms_idn=e.idn And d.Nme_Idn= ? and nvl(c.prt2,'NA')=decode(nvl(c.prt2,'NA'),'SPECIAL',nvl(PACK_VAR.Get_Special,'NA'),nvl(c.prt2,'NA')) \n";
        pktDtlSql+=" and trunc(b.dte)  between "+dtefrom+" and "+dteto+"  order by a.idn,c.sk1 ";
      ary = new ArrayList();
      ary.add(nmeidn);
      ...
      while(rs.next()){
        ...
        pktDtl.put("RAPVLU",util.nvl(rs.getString("rapvlu")));
          ...
          outLst = db.execSqlLst(" Pkt Prp", getPktPrp, ary);
          pst = (PreparedStatement)outLst.get(0);
    ...
    public ActionForward pktDtlExcelXLPNDSUMMY(ActionMapping am, ActionForm form,
        ...
        if(!nmeidn.equals("") && !dte.equals("")){
          ...
          " from mjan a,jandtl b, mstk c,mnme d \n" + 
          "where a.idn=b.idn and b.mstk_idn=c.idn and d.nme_idn=a.nme_idn  and b.stt not in ('APRT','RT','CS','CL','ALC','MRG')  and c.pkt_ty in('NR','SMX') and nvl(c.prt2,'NA')=decode(nvl(c.prt2,'NA'),'SPECIAL',nvl(PACK_VAR.Get_Special,'NA'),nvl(c.prt2,'NA')) \n" + 
          "and b.typ in ('IAP','OAP', 'EAP','WAP','LAP','MAP','SAP','HAP','BAP','KAP','BIDAP') And d.Nme_Idn= ? \n";
  ...
  //          itemHdr.add("QTY");
    ...
    public DailyApproveReportAction() {
        super();
    }
}
