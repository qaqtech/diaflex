...
public class CityAction extends DispatchAction {
    private final String formName   = "cityfrm";
    ...
    public String init(HttpServletRequest req , HttpServletResponse res,HttpSession session,DBUtil util) {
            ...
            System.out.print("New Session Id :="+session.getId());
                ...
                rtnPg=util.checkUserPageRights("masters/country.do?","");
                if(rtnPg.equals("unauthorized"))
                util.updAccessLog(req,res,"City form", "Unauthorized Access");
                else
                util.updAccessLog(req,res,"City form", "init");
            }
            }
            return rtnPg;
         }
    public ActionForward load(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
        ...
        if(!rtnPg.equals("sucess")){
            ...
            util.updAccessLog(req,res,"City form", "load start");
        ...
        String countryIdn = req.getParameter("countryIdn");

        if (countryIdn == null) {
            countryIdn = (String)udf.getValue("countryIdn");
        }
        if (countryIdn == null) {
            countryIdn = (String)info.getValue("countryIdn");
        }
        ArrayList params = new ArrayList();

        params.add(countryIdn);

        String keyQ = "select country_nm from mcountry where country_idn =? ";

        info.setValue("CityCountry", util.getPageKeyNme("city:country", keyQ, params));
        //info.setValue("country_idn", countryIdn);by Mayur
        info.setValue("countryIdn", countryIdn);
        String    srchFields = helper.getSrchFields(daos, "city_idn");
        String    srchQ      = " and country_idn = ? ";
        ...
        udf.setValue("countryIdn", countryIdn);
            util.updAccessLog(req,res,"City form", "load end");
    ...
    public ActionForward save(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
        ...
        if(!rtnPg.equals("sucess")){
            ...
            util.updAccessLog(req,res,"City form", "save start");
            HashMap  formFields = info.getFormFields(formName);
        ...
        String countryIdn = (String) udf.getValue("countryIdn");
        ...
        if (udf.getAddnew() != null) {
            ...
            for (int i = 1; i <= lmt; i++) {
                ...
                ArrayList params   = new ArrayList();
                String insIntoQ = " insert into mcity ( city_idn, country_idn ";
                String insValQ  = "values(city_seq.nextval ,?  ";
        ...
        if (udf.getModify() != null) {
            ...
            String srchFields = helper.getSrchFields(daos, "city_idn", false);
            String srchQ      = " and country_idn = ? ";

            // ArrayList list = helper.getSrchList(uiForms, "", srchFields, srchQ, params, daos, "VIEW");
            String getMenu    = "select " + srchFields + " from mcity where 1 = 1 " + srchQ + " order by 1";
            ArrayList params     = new ArrayList();
            ...
            while (rs.next()) {
                ...
                String updQ  = " update mcity set city_idn = city_idn ";
                String condQ = " where city_idn = ? ";
                boolean isValid = false;
        ...
        if (udf.getDelete() != null) {
            ...
            if(!lIdnlst.equals("")){
                ...
                params  = new ArrayList();
                params.add(countryIdn);
                int ct = db.execUpd(" del msg", "update mcity set vld_dte=sysdate where country_idn=? and city_idn in("+lIdnlst+")", params);
            }
        }
        req.setAttribute("errors", errorList);
            util.updAccessLog(req,res,"City form", "save end");
    ...
    public ActionForward delete(ActionMapping am, ActionForm af, HttpServletRequest req, HttpServletResponse res)
        ...
        if(!rtnPg.equals("sucess")){
            ...
            util.updAccessLog(req,res,"City form", "delete start");
        CountryFrm udf = (CountryFrm) af;
        String cityIdn = req.getParameter("cityIdn");
        String deleteCity = "update mcity set vld_dte=sysdate where city_idn= ?";
        ArrayList ary = new ArrayList();
        ary.add(cityIdn);
        int ct = db.execUpd("deleteCity", deleteCity, ary);
            util.updAccessLog(req,res,"City form", "delete end");
        return load(am, af, req, res);
        }
    }
}
