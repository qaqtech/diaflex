...
<input type="hidden" id="reqUrl" name="reqUrl" value="<%=info.getReqUrl()%>"/>
  ...
  <span class="pgHed">Monthly Comparision Group Wise
  </span></td></tr></table>
  </td></tr>
  ...
  <tr><td valign="top" class="tdLayout">
   ...
   shGrpLst.add("ROUND");shGrpLst.add("FANCY");
    ...
    String bgcolor="";
    String CNT="",CNTGNT="";double per=0.0;
   ...
   <tr><th colspan="<%=1+((szGrpLstsz*3)*2)%>"><%=shape %> - <%=util.nvl((String)mprp.get(lprp+"D"))%> Comparision</th></tr>
   <tr>
   <td></td>
   ...
   <td></td>
   <%for(int sz=0;sz<szGrpLstsz;sz++){%>
   <td align="center" colspan="2">+</td><td align="center" colspan="2">=</td><td align="center" colspan="2">-</td>
   <%}%>
   </tr>
   ...
   <tr>
   <th><%=lprpgrp%> &nbsp;&nbsp;
   <img src="../images/plus.jpg" id="src_<%=shape%>_<%=lprpgrp%>" onclick="MonthlyCmpGrpWise('<%=lprpgrp%>','<%=shape%>','<%=lprp%>',this)" /> 
   </th>
   <%for(int sz=0;sz<szGrpLstsz;sz++){
   ...
   %>
   <a href="monthCmp.do?method=moncmpGrpPKTDTL&sign=<%=dscLstDtl.get(dsc)%>&lprp=<%=lprp%>&grp=<%=lprpgrp%>&szgrp=<%=szgrp%>&shape=<%=shape%>" id="LNK_<%=shape%>_<%=lprp%>" onclick="loadASSFNL('<%=target%>','LNK_<%=shape%>_<%=lprp%>')"  target="SC" title="Click Here to get Packets"><%=CNT%></a>
   <%}%>
   </td>
  ...
  <%}}%>
   ...
   <th>TOTAL
   &nbsp;&nbsp;
   <img src="../images/plus.jpg" id="src_<%=shape%>_TOTAL" onclick="MonthlyCmpGrpWise('TOTAL','<%=shape%>','<%=lprp%>',this)" /> 
   </th>
   <%
   bgcolor="";
   for(int sz=0;sz<szGrpLstsz;sz++){
   ...
   <td nowrap="nowrap" bgcolor="<%=bgcolor%>" align="right">
   <%if(!CNT.equals("0")){%>
   <a href="monthCmp.do?method=moncmpGrpPKTDTL&sign=<%=dscLstDtl.get(dsc)%>&lprp=<%=lprp%>&grp=TOTAL&szgrp=<%=szgrp%>&shape=<%=shape%>" id="LNK_<%=shape%>_<%=lprp%>" onclick="loadASSFNL('<%=target%>','LNK_<%=shape%>_<%=lprp%>')"  target="SC" title="Click Here to get Packets"><%=CNT%></a>
  ...
  <%}}%>
   ...
   <table class="grid1">
   <tr><th colspan="<%=(1+(shGrpLstsz*3)+3)%>">Shape Wise Summary</th></tr>
   <tr>
   <td></td>
   <%for(int s=0;s<shGrpLstsz;s++){%>
