...
%>
  ...
  <img src="../images/line.jpg" border="0" /></td><td valign="middle"> <span class="pgHed">Transaction (Update & Delete) </span> </td>
</tr></table>
  </td>
  </tr> <tr> 
   <td valign="top" class="tdLayout">
   <html:form action="receipt/receiptTransReport.do?method=TransDtl" method="post">
   <table>
   <tr> 
   ...
   <tr> 
   <td valign="top" class="tdLayout"> 
   Create Excel <img src="../images/ico_file_excel.png" onclick="goTo('receiptTransReport.do?method=TransDtlExcel','','')" border="0"/>
   </td></tr>
    <tr> 
   <td valign="top" class="tdLayout"> 
    <table cellpadding="2" cellspacing="2">
    <tr><td><html:submit property="value(update)" value="Update XRT" styleClass="submit" /></td></tr>
    <tr><td><b>Main Transaction</b></td></tr>
    <tr><td>
    <table class="grid1">
   <tr><th><b>Select ALL</b> <input type="checkbox"  value="IS" id="IS" onclick="CheckBOXALLForm('0',this,'cb_trns_')" />&nbsp;</th><th>Party Name</th><th>Trans Dte</th><th>Type</th><th>Sub Type</th><th>Ref Typ</th><th>Ref Idn</th><th>Client ID</th><th>Due Dte</th><th>Credit($)</th><th>Debit($)</th><th>Credit</th><th>Debit</th><th>Xrt</th></tr>
   <% for(int i=0;i<maintransDtlLst.size();i++){
   HashMap transMap = (HashMap)maintransDtlLst.get(i); 
   String ref_idn = util.nvl((String)transMap.get("REF_IDN"));
    ...
    %>  
 <tr>
 <td><input type="checkbox" name="cb_trns_<%=idn%>" value="<%=ref_idn%>" id="cb_trns_<%=idn%>"  /></td>
 <td><%=transMap.get("NME")%></td><td><%=transMap.get("T_DTE")%></td><td><%=transMap.get("TYP")%></td><td><%=transMap.get("SUBTYP")%></td>
  ...
  <td> <input type="text" name="XRT_<%=ref_idn%>" id="XRT_<%=ref_idn%>" value="<%=xrt%>" style="txtStyle" size="6" /></td>
  </tr>
  <% }%>
  </table>
  </td></tr></table></td></tr>
  ...
  <%}
   ...
   <tr><td valign="top" class="tdLayout"><html:submit property="value(delete)" value="Delete Transaction" styleClass="submit" /></td></tr>
  ...
  <b> Sub Transaction</b>
   ...
   <td valign="top" class="tdLayout"> 
    <table class="grid1">
   <tr><th></th><th>Party Name</th><th>Trans Dte</th><th>Type</th><th>Sub Type</th><th>Ref Typ</th><th>Ref Idn</th><th>Due Dte</th><th>Credit($)</th><th>Debit($)</th><th>Credit</th><th>Debit</th><th>Xrt</th></tr>
   <%
   String pEnt_seq="";
   ...
   String ent_seq = util.nvl((String)transMap.get("ENT_SEQ"));
    ...
    if(pEnt_seq.equals(""))
    isChecked=true;
    if(!pEnt_seq.equals(ent_seq))
     isChecked=true;
    
    %>  
 <tr> 
 <%if(isChecked){%>
 <td><input type="checkbox" name="cb_del_<%=idn%>" value="<%=ent_seq%>" id="cb_del_<%=idn%>"  /></td>
 <%}%>
  <td><%=transMap.get("NME")%></td><td><%=transMap.get("T_DTE")%></td><td><%=transMap.get("TYP")%></td><td><%=transMap.get("SUBTYP")%></td>
 ...
 <td align="right"><b><%=(String)session.getAttribute("TTLUCR")%></b></td> <td align="right"><b><%=(String)session.getAttribute("TTLUDB")%></b></td>
 <td align="right"><b><%=util.priceFormatter((String)session.getAttribute("TTLCR"))%></b></td> <td align="right"><b><%=util.priceFormatter((String)session.getAttribute("TTLDB"))%></b></td>
  <td></td></tr>
  </table>
  </td></tr>
 <%}else{%>
