...
<script type="text/javascript" src="<%=info.getReqUrl()%>/scripts/ajax.js?v=<%=info.getJsVersion()%> " > </script>
  ...
  <title>Daily Web Request Report</title>
...
Daily Web Request Report
  ...
  <td valign="top" class="hedPg">
    <html:form action="report/dailyWebRequestAction.do?method=load" method="post" >
    ...
    <html:text property="value(dteto)" styleClass="txtStyle"  styleId="dteto"  size="12"  /> <a href="#"  onClick="setYears(1900, '<%=info.getCurrentYear()%>' );showCalender(this, 'dteto');">
      ...
      %>
      <html:select name="dailyWebRequestForm" property="value(styp)" styleId="saleEmp">
...
ArrayList grpList = ((ArrayList)info.getGroupcompany() == null)?new ArrayList():(ArrayList)info.getGroupcompany();
   ...
   %>
      ...
      <td>Group Company</td>
      <td><html:select name="dailyWebRequestForm" property="value(group)" styleId="grp">
      ...
      if(pageList!=null && pageList.size() >0){%>
      <td>Memo Type</td>
      <td><html:select property="value(typ)"  name="dailyWebRequestForm">
      <html:option value="">----select----</html:option>
  ...
  &nbsp;&nbsp;&nbsp;Create Excel <img src="../images/ico_file_excel.png" onclick="goToCreateExcel('dailySalesReport.do?method=pktDtlExcel','','')" border="0"/> 
   ...
   <%pageList=(ArrayList)pageDtl.get("PKTDTL");
      ...
      pageDtlMap=(HashMap)pageList.get(j);
       ...
       fld_ttl = util.nvl((String)pageDtlMap.get("fld_ttl"));
       if(dflt_val.equals("")){
       dflt_val ="dailyWebRequestAction.do?method=pktDtlExcel"; 
      ...
      <%}}%>
                  ...
                  <% for(int k=0;k<memotyp.size();k++){
                        ...
                        if(dtl!=null && dtl.size()> 0){
                            for(int l=k;l<=k;l++){
                            %>
                            <td align="center"><a onclick="callWRReportPkt('<%=byr_idn.get(emp+"_"+byr)%>','<%=loop%>','<%=memotyp.get(k)%>')" title="Click Here To See Details" style="text-decoration:underline"><%=dtl.get(0)%></a> </td>
