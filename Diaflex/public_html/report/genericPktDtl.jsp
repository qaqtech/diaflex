...
<iframe name="SC" class="frameStyle" align="middle" frameborder="0">
 ...
 <td>
  <span class="txtLink" onclick="GenericRT('SOLD_')" >Sold</span>
  </td>
  <%}}%>
...
<td height="20">
<div align="center">
<%=util.nvl((String)sttMap.get("ZQ"))%>

</div>
</td>
<td><div align="center">
<%=util.nvl((String)sttMap.get("ZC"))%>

</div></td>

<td><div align="center">
<%=util.nvl((String)sttMap.get("ZA"))%>
</div></td>

<td><div align="center">
<%=util.nvl((String)sttMap.get("ZD"))%>
</div></td>
<td><div align="center">
<%=util.nvl((String)sttMap.get("ZSD"))%>
</div></td>
<td><div align="center">
<%=util.nvl((String)sttMap.get("ZV"))%>
</div></td>
<%pageList=(ArrayList)pageDtl.get("SELECTED");
if(pageList!=null && pageList.size() >0){
for(int j=0;j<pageList.size();j++){
...
fld_nme=(String)pageDtlMap.get("fld_nme");%>
<td><div align="center">
<%=util.nvl((String)sttMap.get("Z"+fld_ttl))%>
</div></td>
<%}}%>
</tr>
...
<td height="20">
<div align="center">
<%=util.nvl((String)sttMap.get(selectidstt+"Q"))%>

</div>
</td>
<td><div align="center">
<%=util.nvl((String)sttMap.get(selectidstt+"C"))%>

</div></td>

<td><div align="center">
<%=util.nvl((String)sttMap.get(selectidstt+"A"))%>
</div></td>

<td><div align="center">
<%=util.nvl((String)sttMap.get(selectidstt+"D"))%>
...
<td><div align="center">
<%=util.nvl((String)sttMap.get(selectidstt+"V"))%>
...
defaultstt=status;
%>
<tr id="<%=status%>_" style="">
<td><div align="center"><strong>Total</strong></div></td>
<td height="20">
<div align="center">
<%=util.nvl((String)sttMap.get(status+"Q"))%>

</div>
</td>
<td><div align="center">
<%=util.nvl((String)sttMap.get(status+"C"))%>

</div></td>

<td><div align="center">
<%=util.nvl((String)sttMap.get(status+"A"))%>
</div></td>

<td><div align="center">
<%=util.nvl((String)sttMap.get(status+"D"))%>
...
<td><div align="center">
<%=util.nvl((String)sttMap.get(status+"V"))%>
...
<%if(statusLst.contains("SOLD") && !status.equals("SOLD") && !status.equals("NG")){%>
<tr id="SOLD_" style="display:none">
<td><div align="center"><strong>Total</strong></div></td>
<td height="20">
<div align="center">
<%=util.nvl((String)sttMap.get("SOLDQ"))%>

</div>
</td>
<td><div align="center">
<%=util.nvl((String)sttMap.get("SOLDC"))%>

</div></td>

<td><div align="center">
<%=util.nvl((String)sttMap.get("SOLDA"))%>
</div></td>

<td><div align="center">
<%=util.nvl((String)sttMap.get("SOLDD"))%>
...
<td><div align="center">
<%=util.nvl((String)sttMap.get("SOLDV"))%>
...
<%if(((statusLst.contains("SOLD") || status.equals("SOLD")) || psearch.equals("Y"))  && !status.equals("NG")){%>
<td>
<table cellpadding="2" cellspacing="2">
<tr><td>
<%

    pageList=(ArrayList)pageDtl.get("WEEKS");
      if(pageList!=null && pageList.size() >0){
      for(int j=0;j<pageList.size();j++){
      pageDtlMap=(HashMap)pageList.get(j);
      ...
      <B>Summary</b>&nbsp;&nbsp;WEEKS : <html:text property="<%=fld_nme%>" size="2" name="genericReportForm" styleId="<%=fld_typ%>" value="<%=dflt_val%>"/>
    <%}}
    %>

<!--&nbsp;&nbsp;WEEKS : <html:text property="value(WEEKS)" styleId="WEEKS" size="2" name="genericReportForm"/>-->
</td>
</tr>
...
<tr><td>
<table>
<input type="hidden" name="weekshidden" id="weekshidden" value=""/>
 <input type="hidden" name="sltStt" id="sltStt" value="MKT" />
 <html:hidden property="value(row)" name="genericReportForm" />
  <html:hidden property="value(col)" name="genericReportForm" />
<%  pageList=(ArrayList)pageDtl.get("LINKNEW");
if(pageList!=null && pageList.size() >0){%>
<tr>
<td id="summary">
...
<li>
<a href="#"><B style="color: Black;">Summary Details.....</b></a>
<ul>
<%
for(int j=0;j<pageList.size();j++){
pageDtlMap=(HashMap)pageList.get(j);
fld_nme=(String)pageDtlMap.get("fld_nme");
dflt_val=(String)pageDtlMap.get("dflt_val");
dflt_val=dflt_val.replaceAll("KEY",key);
dflt_val=dflt_val.replaceAll("ROW",row);
dflt_val=dflt_val.replaceAll("COL",col);
fld_ttl=(String)pageDtlMap.get("fld_ttl");
val_cond=(String)pageDtlMap.get("val_cond");
flg1=(String)pageDtlMap.get("flg1");
if(!flg1.equals("MKT")){%>
<li><A href="<%=dflt_val%>" id="<%=fld_nme%>"  onclick="<%=val_cond%>" target="SC"><span><%=fld_ttl%></span></a></li>
<%}else if(flg1.equals("MKT")){
if(statusLst.contains("MKT") && (status.equals("MKT") || status.equals("ALL"))){%>
<li><A href="<%=dflt_val%>" id="<%=fld_nme%>"  onclick="<%=val_cond%>" target="SC"><span><%=fld_ttl%></span></a></li>
<%}}}%>
</ul>
</li>
...
<%}%>
  ...
  String callAnalysisGrpby="callAnalysisGrpby('"+key+"','"+orderbyrow+"','"+orderbycol+"','"+status+"')";
            pageList=(ArrayList)pageDtl.get("GRPBY");
             if(pageList!=null && pageList.size() >0){%>
             Group by <html:select property="value(grpby)" styleId="grpby" onchange="<%=callAnalysisGrpby%>" name="genericReportForm" >
  ...
  callAnalysisGrpby="callAnalysisOrderby('"+key+"','"+orderbyrow+"','"+orderbycol+"','"+status+"')";
            pageList=(ArrayList)pageDtl.get("ORDERBY");
             if(pageList!=null && pageList.size() >0){%>
             Order by <html:select property="value(orderby)" styleId="orderby" onchange="<%=callAnalysisGrpby%>" name="genericReportForm" >
             <html:option value="">--Select--</html:option>
             <%
               for(int j=0;j<pageList.size();j++){
                 pageDtlMap=(HashMap)pageList.get(j);
...
<div align="left"><input type="radio" name="RD_<%=prp%>" onclick="CalculateAvgTotal('<%=prp%>','<%=defaultstt%>','<%=fprp%>','AVG')" value="AVG" />&nbsp;Average</div>
...
<div align="left"><input type="radio" name="RD_Fnlpri" onclick="CalculateAvgTotal('Fnlpri','<%=defaultstt%>','Fnlpri','AVG')" value="AVG" />&nbsp;Average</div>
...
<div align="left"><input type="radio" name="RD_<%=fld_nme%>" onclick="CalculateAvgTotal('<%=fld_nme%>','<%=defaultstt%>','<%=lov_qry%>','AVG')" value="AVG" />&nbsp;Average</div>
<%if(val_cond.equals("")){%>
