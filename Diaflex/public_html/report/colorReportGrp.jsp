...
<link type="text/css" rel="stylesheet" href="<%=info.getReqUrl()%>/css/style.css?v=<%=info.getJsVersion()%>"/>
 ...
 <body onfocus="<%=onfocus%>" onload="MM_preloadImages('images/faunalogo.png')" topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0">
   <%
    ...
    clrLst=(ArrayList)reportDtl.get("CO_HDR");
    coList=(ArrayList)reportDtl.get("CO_CONTENT");
    dataTbl=(HashMap)reportDtl.get("CO_DATA");
    }
    ...
    String sz= (String)request.getAttribute("sz");
...
<bean:write property="reportNme" name="groupWiseForm" />
  ...
  <tr><td valign="top" class="hedPg">Create Excel <img src="../images/ico_file_excel.png" title="Click here to Generate Excel" onclick="goToCreateXl('groupGrp.do?method=createGridXL&excelNm=Color/Purity&Report=CO')" border="0"/>&nbsp; 
  ...
  <tr>
  <th>Color/Purity</th>
  <%for(int i=0;i<clrLst.size();i++){%>
  <th colspan="8"><%=clrLst.get(i)%></th>
  <%}%>
  ...
  String col=(String)coList.get(j);
  String dsc=util.nvl2((String)dataTbl.get(col),col);
  %>
  <tr>
  <th><%=dsc%></th>
  <%for(int k=0;k<clrLst.size();k++){
  String clr=(String)clrLst.get(k);
  data=new HashMap();
  Qty="";
  Cts="";
  Avg="";
  ...
  Mfgpri="";
   age="";
  data=(HashMap)dataTbl.get(col+"_"+clr);
  ...
  %>
  <td align="right" style="padding:0px"><%=Qty%></td>
  <td align="right" style="padding:0px"><span id="CTS_<%=j%>_<%=k%>" style="display:none;"><a title="Packet Details" target="_blank"  href="groupGrp.do?method=pktDtl&group=<%=group%>&lab=<%=lab%>&stt=<%=stt%>&sh=<%=sh%>&sz=<%=sz%>&col=<%=col%>&clr=<%=clr%>&loc=<%=locS%>&mlot=<%=mlot%>" ><%=Cts%></a></span></td>
  ...
  Mfgpri="";
    age="";
  data=new HashMap();
  data=(HashMap)dataTbl.get(col+"_TTL");
  if(data!=null && data.size()>0){
  Qty=util.nvl((String)data.get("QTY"));
  Cts=util.nvl((String)data.get("CTS"));
  Avg=util.nvl((String)data.get("AVG"));
  ...
  %>
  <td align="right" style="padding:0px"><%=Qty%></td>
   <td align="right" style="padding:0px"><span id="TOTALCTS_<%=j%>" style="display:none;"><a title="Packet Details" target="_blank"  href="groupGrp.do?method=pktDtl&group=<%=group%>&lab=<%=lab%>&stt=<%=stt%>&sh=<%=sh%>&sz=<%=sz%>&col=<%=col%>&clr=ALL&loc=<%=locS%>&mlot=<%=mlot%>" ><%=Cts%></a></span></td>
  ...
  <th>Total</th>
   <%for(int i=0;i<clrLst.size();i++){
  String clr=(String)clrLst.get(i); 
  Qty="";
  Cts="";
  Avg="";
  Dis="";
  ...
  Mfgpri="";
    age="";
  data=new HashMap();
  data=(HashMap)dataTbl.get(clr+"_TTL");
  if(data!=null && data.size()>0){
  Qty=util.nvl((String)data.get("QTY"));
  Cts=util.nvl((String)data.get("CTS"));
  Avg=util.nvl((String)data.get("AVG"));
  ...
  Fapri=util.nvl((String)data.get("FAPRI"));
  Mfgpri=util.nvl((String)data.get("MFGPRI"));
    age=util.nvl((String)data.get("MFGPRI"));
  }
  %>
  <td align="right" style="padding:0px"><%=Qty%></td>
  <td align="right" style="padding:0px"><span id="TTLCTS_<%=i%>" style="display:none;"><a title="Packet Details" target="_blank"  href="groupGrp.do?method=pktDtl&group=<%=group%>&lab=<%=lab%>&stt=<%=stt%>&sh=<%=sh%>&sz=<%=sz%>&col=ALL&clr=<%=clr%>&loc=<%=locS%>&mlot=<%=mlot%>" ><%=Cts%></a></span></td>
  ...
  %>
  <td align="right" style="padding:0px"><%=Qty%></td>
   <td align="right" style="padding:0px"><span id="GTTLCTS" style="display:none;"><a title="Packet Details" target="_blank"  href="groupGrp.do?method=pktDtl&group=<%=group%>&lab=<%=lab%>&stt=<%=stt%>&sh=<%=sh%>&sz=<%=sz%>&col=ALL&clr=ALL&loc=<%=locS%>&mlot=<%=mlot%>" ><%=Cts%></a></span></td>
