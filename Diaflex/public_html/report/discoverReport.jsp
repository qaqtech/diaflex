...
<table width="100%"   align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <html:form action="report/discover.do?method=fecth" method="post" onsubmit="return validate_assort()">
...
<tr>
<td>Packet Id: </td>
<td><html:textarea property="value(vnm)" name="discoverReportForm" cols="40" rows="8" styleId="pid"/></td>
</tr>
<tr>
<td>Packet Type</td>
<td>
<html:select property="value(pkt_ty)" name="discoverReportForm">
...
<td>
 ...
 <% for(int n=0;n<sttList.size();n++){
  ...
  String chkid="STT_"+n;
  String onclick="getStatusbyGrp('"+grp1+"','"+chkid+"')";
  if(counter==4){%>
  </tr><tr>
  <%}counter++;%>
  <td><html:checkbox property="<%=dscgrp1%>" styleId="<%=chkid%>" value="<%=grp1%>" name="discoverReportForm" onclick="<%=onclick%>"/></td><td><%=dsc%></td>
  <%}%>
...
<%}%>
    ...
    <td valign="top" align="center">Status :<html:textarea property="value(stt)" cols="40" rows="2" styleId="sttLst" value="<%=dfltstt%>" name="discoverReportForm" /> </td>
  </tr>
<tr>
...
ArrayList rolenmLst=(ArrayList)info.getRoleLst();
 ...
 <tr><td valign="top" class="hedPg">
  Create Excel &nbsp;&nbsp;&nbsp;
  <img src="../images/ico_file_excel.png" border="0" onclick="goTo('<%=info.getReqUrl()%>/report/discover.do?method=createXL','','')"/>
 
  </td></tr>
<tr><td valign="top" class="hedPg">

<display:table name="sessionScope.discoverReportForm.pktList" requestURI=""  export="true" class="displayTable" >
  ...
  <display:setProperty name="export.excel.filename" value="StockDetails.xls"/>
  <display:setProperty name="export.pdf.filename" value="StockDetails.pdf"/>
  <display:setProperty name="export.csv.filename" value="StockDetails.csv"/>
