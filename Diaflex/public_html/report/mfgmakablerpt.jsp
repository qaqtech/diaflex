...
<script type="text/javascript" src="<%=info.getReqUrl()%>/scripts/ajax.js?v=<%=info.getJsVersion()%> " > </script>
  ...
  <%
    ...
    ArrayList blckLst= ((ArrayList)mfgprpset.get("blckLst") == null)?new ArrayList():(ArrayList)mfgprpset.get("blckLst");
    ...
    ArrayList orderLst= ((ArrayList)mfgprpset.get("orderLst") == null)?new ArrayList():(ArrayList)mfgprpset.get("orderLst");
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <img src="../images/line.jpg" border="0" /></td><td valign="middle"> <span class="pgHed">Makable Report</span> </td>
</tr></table>
  </td>
  </tr>
<tr><td valign="top" class="hedPg">
<html:form action="/report/orclRptAction.do?method=fetchmakablerpt"  method="POST">
  <table  class="grid1">
    <tr><th colspan="2">Generic Search</th></tr>
    <tr>
  <td>Invoice id</td>
  <td>
  <html:select  property="PInvDocIdLst" styleId="P_Inv_Doc_Id" multiple="true" style=" height:80px;" size="4"  >
  <html:option value="">All</html:option>
  <%for(int j=0; j < docLst.size(); j++) {
    String pPrt = util.nvl((String)docLst.get(j));
    %>
  ...
  <td>Block</td>
  <td>
  <html:select  property="value(blck)" styleId="blck"  >
    <html:option value="">All</html:option>
  <%for(int j=0; j < blckLst.size(); j++) {
    String pPrt = util.nvl((String)blckLst.get(j));
    %>
  <html:option value="<%=pPrt%>"><%=pPrt%></html:option>
  <%}%>
  </html:select>
...
<%}}}}%>
  ...
  <td>Order 1</td>
  <td>
  <html:select  property="value(order_1)" styleId="order_1"  >
  <%for(int j=0; j < orderLst.size(); j++) {
    String pPrt = util.nvl((String)orderLst.get(j));
    %>
  <html:option value="<%=pPrt%>"><%=pPrt%></html:option>
  <%}%>
  </html:select>
  ...
  <td>Order 2</td>
  <td>
  <html:select  property="value(order_2)" styleId="order_2"  >
  <%for(int j=0; j < orderLst.size(); j++) {
    String pPrt = util.nvl((String)orderLst.get(j));
    %>
  <html:option value="<%=pPrt%>"><%=pPrt%></html:option>
...
<td>
<table class="grid1">
<tr><th colspan="6"> Summary Details</th></tr>
<tr>
<th>Invoice Id</th>
<th>Lot Number</th>
<th>Pcs</th>
<th>Making Carets</th>
<th>Exp Polish Caret</th>
<th>Value</th>
</tr>
...
<%
    ...
    %>
    <tr>
    <td align="center"><%=util.nvl((String)pktDtlMap.get("Inv_Doc_Id"))%></td>
    <td align="center"><%=util.nvl((String)pktDtlMap.get("Lt_Nmbr"))%></td>
    <td  align="right"><%=util.nvl((String)pktDtlMap.get("pckt_nmbr"))%></td>
    <td  align="right"><%=util.nvl((String)pktDtlMap.get("Mk_Crts"))%></td>
    <td  align="right"><%=util.nvl((String)pktDtlMap.get("Exp_Plsh_Crts"))%></td>
...
<td valign="top" class="hedPg">
<table class="grid1">
<tr><th colspan="15"> Packet Details</th></tr>
<tr>
<th>Invoice Id</th>
<th>Lot Number</th>
<th>Packet Number</th>
...
<%
    ...
    <td align="center"><%=util.nvl((String)pktDtlMap.get("Lt_Nmbr"))%></td>
    <td align="right"><%=util.nvl((String)pktDtlMap.get("Pckt_Nmbr"))%></td>
    <td align="right"><%=util.nvl((String)pktDtlMap.get("Empl_Cd"))%></td>
    <td align="right"><%=util.nvl((String)pktDtlMap.get("Blck_Cd"))%></td>
    <td align="right"><%=util.nvl((String)pktDtlMap.get("Mk_Crts"))%></td>
    <td align="right"><%=util.nvl((String)pktDtlMap.get("Exp_Plsh_Crts"))%></td>
    <td align="right"><%=util.nvl((String)pktDtlMap.get("Vlu"))%></td>
    ...
    <!--<td><%=util.nvl((String)pktDtlMap.get("Tn"))%></td>-->
    <td><%=util.nvl((String)pktDtlMap.get("Mk_Dt"))%></td>
    </tr>
    <%}%>
    </table>
    </td>
