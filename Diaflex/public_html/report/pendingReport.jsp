...
<html>
 ...
 <meta http-equiv="Content-Type" content="text/html; charset=windows-1252"/>
  ...
  <%
   ArrayList repMemoLst =(ArrayList)session.getAttribute("DLV_PND_VW");
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <tr>
  <td valign="top" class="hedPg">
  <html:form action="report/orclReportAction.do?method=fetchDlvPnd" method="post">
...
<%}if(flg1.equals("SDATE")){%>
  <tr><td><%=fld_nme%></td>
  <td>
    <table><tr>
      <td>From</td><td>
        <div class="float"> <html:text property="value(saleDte)"  styleId="saleDte" /></div>
        <div class="float"> <a href="#"  onClick="setYears(1900, <%=info.getCurrentYear()%>);showCalender(this, 'saleDte');">
          <img src="<%=info.getReqUrl()%>/images/calender.png"></a></div>
      </td>
      <td>To</td><td>
        <div class="float"> <html:text property="value(saletoDte)"  styleId="saletoDte" /></div>
        <div class="float"> <a href="#"  onClick="setYears(1900, <%=info.getCurrentYear()%>);showCalender(this, 'saletoDte');">
          <img src="<%=info.getReqUrl()%>/images/calender.png"></a></div>
      </td>
    </tr></table>
  </td>
  </tr><%}if(flg1.equals("DDATE")){%>
    <tr><td><%=fld_nme%></td>
  <td>
    <table><tr>
      <td>From</td><td>
        <div class="float"> <html:text property="value(dlvDte)"  styleId="dlvDte" /></div>
        <div class="float"> <a href="#"  onClick="setYears(1900, <%=info.getCurrentYear()%>);showCalender(this, 'dlvDte');">
          <img src="<%=info.getReqUrl()%>/images/calender.png"></a></div>
      </td>
      <td>To</td><td>
        <div class="float"> <html:text property="value(dlvtoDte)"  styleId="dlvtoDte" /></div>
        <div class="float"> <a href="#"  onClick="setYears(1900, <%=info.getCurrentYear()%>);showCalender(this, 'dlvtoDte');">
          <img src="<%=info.getReqUrl()%>/images/calender.png"></a></div>
      </td>
    </tr></table>
  </td>
  ...
  <table><tr><td>
  <img src="../images/edit.jpg" title="Edit Properties Module" onclick="newWindow('<%=info.getReqUrl()%>/dynamicProperty.do?method=load&mdl=DLV_PND_VW&sname=DLV_PND_VW&par=A')" border="0" width="15px" height="15px"/>
 </td><td>
 <% String qty = (String)request.getAttribute("qty");
if(qty!=null){
String cts = (String)request.getAttribute("cts");
String vlu = (String)request.getAttribute("vlu");%>
 <table>
   <tr><td>Total :&nbsp;&nbsp;</td><td><span id="ttlqty"><%=qty%></span> &nbsp;&nbsp;</td><td>cts&nbsp;&nbsp;</td><td><span id="ttlcts"> <%=cts%></span> &nbsp;&nbsp;Value:</td><td><span id="ttlcts"> <%=vlu%></span> &nbsp;&nbsp;</td> </tr>
...
<%}%></td></tr></table>
  ...
  <display:table name="sessionScope.orclReportForm.pktList" class="displayTable" requestURI="" export="true"  sort="list" decorator="ft.com.Decortor">
   <%if(cnt.equals("ri")){%>
   <display:column property="Customer ID"  title="Customer ID" group="3" />
   <%}%>
   <display:column property="sal" style="font-weight: bold;" title="Sale Person" group="1" />
    <display:column property="byrnme" style="font-weight: bold;" title="Buyer" group="2" />
     <display:column property="rmk" style="font-weight: bold;" title="Remark" group="3" />
    <display:column property="memoIdn"  title="Id" group="3" />
    <display:column property="vnm"  title="Packet No."  />
    <display:column property="app_dte"  title="Approve Date"  />
