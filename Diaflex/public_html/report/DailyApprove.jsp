...
<script type="text/javascript" src="<%=info.getReqUrl()%>/scripts/ajax.js?v=<%=info.getJsVersion()%> " > </script>
  ...
  <title>Daily Approve Report</title>
...
Daily Approve Report
   ...
   <td valign="top" class="hedPg">
    <html:form action="report/dailyapproveReport.do?method=load" method="post" onsubmit="return verifyDailyApproveRpt();" >
    ...
    <html:hidden property="value(memotbl)" styleId="memotbl" name="dailyApproveReportForm" />
    ...
    <html:text property="value(dteto)" styleClass="txtStyle"  styleId="dteto"  size="12"  /> <a href="#"  onClick="setYears(1900, '<%=info.getCurrentYear()%>' );showCalender(this, 'dteto');">
      ...
      %>
      <html:select name="dailyApproveReportForm" property="value(styp)" styleId="saleEmp">
 ...
 <input type="hidden" name="nmeID" id="nmeID" value="">
  ...
  ArrayList grpList = ((ArrayList)info.getGroupcompany() == null)?new ArrayList():(ArrayList)info.getGroupcompany();
   ...
   %>
      ...
      <td nowrap="nowrap"><html:select name="dailyApproveReportForm" property="value(group)" styleId="grp">
      ...
      <td nowrap="nowrap">
     <html:select property="value(typ)" styleId="typId" name="dailyApproveReportForm">
             <html:option value="">--Select--</html:option>
             <%
  ...
  %>
   ...
   <table class="grid1" width="60%">
     ...
     <% for(int j=0;j<pageList.size();j++){
      pageDtlMap=(HashMap)pageList.get(j);
       ...
       if(dflt_val.equals("")){
       dflt_val ="dailyapproveReport.do?method=pktDtlExcel"; 
   ...
   %>
    ...
    <%  }
      ...
      <a onclick="callMemoApproveReportPkt('<%=util.nvl((String)approvedtl.get("byrid"))%>','<%=loop%>','DAP','<%=cnt%>','<%=(util.nvl((String)approvedtl.get("dte"))).trim() %>')" title="Click Here To See Details" style="text-decoration:underline">
