...
<%
//util.updAccessLog(request,response,"Odd Path Edit", "Odd Path Edit");
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
  ...
  <img src="../images/line.jpg" border="0" /></td><td valign="middle"> <span class="pgHed">Lab Service</span> </td>
</tr></table>
  </td>
  </tr>
  <%
   ...
   <table><tr><td>
   <html:form action="assort/oddPathIssue.do?method=fecth" method="post" onsubmit="return validate_assort()">
  <table  class="grid1">
  <tr><th> </th><th>Generic Search </th></tr>
  <tr><td valign="top">
   <table><tr><td>Process </td>
   <td>
   <html:select property="value(mprcIdn)"  styleId="mprcIdn" name="oddPathEditIssueForm" disabled="<%=disabled%>"   >
           <html:option value="0" >--select--</html:option>
            <html:optionsCollection property="mprcList" name="oddPathEditIssueForm" 
   ...
   <tr>
    ...
    Employee : </td>
   <td>
   <html:select property="value(empIdn)"  styleId="empIdn" name="oddPathEditIssueForm" disabled="<%=disabled%>"  >
           <html:option value="0" >--select--</html:option>
            <html:optionsCollection property="empList" name="oddPathEditIssueForm" 
            label="emp_nme" value="emp_idn" />
            </html:select>
   </td>
   </tr>
   <tr>
   <td>Issue Id</td><td><html:text property="value(issueIdn)" name="oddPathEditIssueForm" /> </td>
   </tr>
   <tr>
   <td>Packet No:</td><td> <html:textarea property="value(vnmLst)" rows="7"  cols="30" name="oddPathEditIssueForm"  /></td>
   </tr>
  </table></td>
   <td>
   ...
   <html:form action="assort/oddPathIssue.do?method=Issue" method="post" onsubmit="return validate_issue('CHK_' , 'count')">
   <html:hidden property="value(prcId)" name="oddPathEditIssueForm" />
   <html:hidden property="value(empId)" name="oddPathEditIssueForm" />
...
<%for(int j=0; j < prpList.size(); j++ ){
    ...
    <%if(prpVal!=null && prpVal.size()>0){%>
     <html:select  property="<%=prpFld%>" styleId="<%=selectALL%>" name="oddPathEditIssueForm" onchange="<%=chksrvAll%>"  style="width:75px"   > 
 ...
 %>
<tr>
<td><html:checkbox property="<%=checkFldVal%>" styleId="<%=checkFldId%>" name="oddPathEditIssueForm" onclick="<%=onclick%>" value="yes" /> </td>
<td><%=sr%></td>
...
<%for(int j=0; j < prpList.size(); j++ ){
    ...
    <td>
    <%if(prpVal!=null && prpVal.size()>0){%>
     <html:select  property="<%=prpFld%>" styleId="<%=selectApp%>" name="oddPathEditIssueForm"  style="width:75px"   > 
     ...
     <%}else{%>
     <html:text property="<%=prpFld%>"  styleClass="txtStyle" name="oddPathEditIssueForm"  style="width:75px"/>
     <%}%>
    </td>
    <%}%>
</tr>
