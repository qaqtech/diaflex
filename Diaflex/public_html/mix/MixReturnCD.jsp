...
<link type="text/css" rel="stylesheet" href="<%=info.getReqUrl()%>/css/general.css?v=<%=info.getJsVersion()%>"/>
  ...
  <script type="text/javascript" src="<%=info.getReqUrl()%>/scripts/newCalScript.js?v=<%=info.getJsVersion()%>" ></script>
    ...
    <title>Assort Return</title>
        ...
        ArrayList boxTypList = (ArrayList)gtMgr.getValue(lstNme+"BOXLST");
        HashMap mixDataMap = (HashMap)gtMgr.getValue(lstNme);
        HashMap stockMap = (HashMap)gtMgr.getValue(lstNme+"_CNLST");
...
<table class="grid1">
<tr><th>Box Typ</th>
<th>Employee</th><th>Qty</th><th>Cts</th><th>Avg Rate.</th>

</tr>
<%for(int i=0;i<boxTypList.size();i++){
 String boxTyp = (String)boxTypList.get(i);
  String fldId = "CHK_"+boxTyp;
   ArrayList mixDataList =  (ArrayList)mixDataMap.get(boxTyp);
 if(mixDataList!=null && mixDataList.size()>0){
 for(int j=0;j<mixDataList.size();j++){
 HashMap mixMap = (HashMap)mixDataList.get(j);
 String emp = (String)mixMap.get("EMP");
 String emp_idn = (String)mixMap.get("EMP_IDN");
  String cts = (String)mixMap.get("CTS");
   String qty = (String)mixMap.get("QTY");
    String avg = (String)mixMap.get("AVG");
 %>
<tr>
 <td>
<% if(j==0){%>
 <%=boxTyp%><%}%>
  ...
  <td><a href="assortCDReturn.do?method=fetch&emp_idn=<%=emp_idn%>&box_typ=<%=boxTyp%>">
  <%=emp%></a>
  </td><td><%=qty%></td><td><%=cts%></td><td><%=avg%></td>
</tr>
<%}}}%>
...
gtMgr.setValue(lstNme+"STTLST", sttLst);
  ...
  %>
      <html:form action="/mix/assortCDReturn.do?method=Return" method="post" onsubmit="return verifedTtlVal()" >
 <tr>
  ...
  String ctsId="",ctsVal="",qtyId="",qtyVal="",prcId="",prcVal="",rteVal="",diffId="",diffVal="",oldCts="",oldCtsVal="";  
