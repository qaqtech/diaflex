...
<html>
  <head>
     <%
  ArrayList  dtlList = (ArrayList)request.getAttribute("dtlList_inward");
  String hdrnme = (String)request.getAttribute("hdrnme_inward");
   String  dsalerfsh = (String)request.getAttribute("dinwardrfsh");
...
if(dtlList!=null && dtlList.size()>0){
  ...
  <tr>
  <th>Lot No</th><th>Shape</th><th>Qty</th><th>Cts</th>
  </tr>
  ...
  byrdata=(HashMap)dtlList.get(i);
   %>
  <tr>
   <td><%=util.nvl((String)byrdata.get("LOTNO"))%></td>
   <td><%=util.nvl((String)byrdata.get("SHAPE"))%></td>
    <td align="right"><%=util.nvl((String)byrdata.get("QTY"))%></td>
    <td align="right"><%=util.nvl((String)byrdata.get("CRTWT"))%></td>
  </tr>
  <%}%>
  </table></div>
  </td></tr>
