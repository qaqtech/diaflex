...
<html>
  <head>
       <%
  ArrayList itemHdr = (ArrayList)request.getAttribute("itemHdr_CR");
  ArrayList dtlsList = (ArrayList)request.getAttribute("dtlList_CR");
  String hdrnme = (String)request.getAttribute("hdrnme_CR");
   String  cabinreqrfsh = (String)request.getAttribute("cabinreqrfsh");
  String  view = (String)request.getAttribute("view");%>
    ...
    <link type="text/css" rel="stylesheet" href="<%=info.getReqUrl()%>/css/style.css?v=<%=info.getJsVersion()%>"/> 
        <script src="<%=info.getReqUrl()%>/scripts/jquery.min.js"></script>
        <link rel="stylesheet" href="<%=info.getReqUrl()%>/css/jScrollbar.jquery.css" type="text/css" />
	<script src="<%=info.getReqUrl()%>/scripts/jquery.mousewheel.js"></script>
	<script src="<%=info.getReqUrl()%>/scripts/jquery-ui-draggable.js"></script>
        <script src="<%=info.getReqUrl()%>/scripts/jscroll.js"></script>
...
<tr><td valign="top"><b style="font-size:10px;"><%=hdrnme%>  </b></td></tr>
      ...
      <div id="main">
    <display:table name="sessionScope.userrightsform.pktCRList" requestURI=""   sort="list" class="boardgrid"  style="width:100%;" >
    ...
    <%}else{
    %>
    <display:column property="<%=hdr%>"   style="text-align:right;"  title="<%=hdr%>"  />
    <%}}%>
