...
<html>
  <head>
     <%
  ArrayList  pktList = (ArrayList)request.getAttribute("dataDtl_DFWEBDTL");
  String hdrnme = (String)request.getAttribute("hdrnme_DFWEBDTL");
   String  dfwebdtlrfsh = (String)request.getAttribute("dfwebdtlrfsh");
...
<%if(view.equals("Y")){
  ...
  <tr>
  <th>Application</th><th>Count</th>
  </tr>
  <%for (int i = 0; i< pktList.size(); i++) {
  HashMap dfwebdata = new HashMap();
  dfwebdata=(HashMap)pktList.get(i);
  %>
  <tr>
  <td nowrap="nowrap" width="50%"><%=util.nvl((String)dfwebdata.get("TYP"))%></td>
  <td align="right" width="50%"><%=util.nvl((String)dfwebdata.get("CNT"))%></td>
  </tr>
  <%}%>
  </table></div>
  </td></tr>
