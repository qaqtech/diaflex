...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg">
 ...
 <% if(request.getAttribute("SLMSG")!=null){%>
  <tr><td valign="top" class="tdLayout">
   <span class="redLabel"> <%=request.getAttribute("SLMSG")%></span>
  </td></tr>
 <%}%>
 ...
 <tr><td valign="top" class="tdLayout">
  ...
  <tr><td><table><tr><td>
  <table><tr><td>
  <html:form action="/mixAkt/mixDeliveryAction.do?method=loadPkt" method="POST" onsubmit="return validate_sale()">
...
<td>Buyer Name :</td>
<td>
 <html:select property="byrIdn" name="mixDeliveryForm" onchange="getFinalByrMix(this,'SL')" styleId="byrId" >
    <html:option value="0">---select---</html:option>
    <html:optionsCollection property="byrLst" name="mixDeliveryForm"  value="byrIdn" label="byrNme" />
...
<html:select property="prtyIdn" name="mixDeliveryForm" onchange="getSaleTrmsMix(this.value , 'SL')"  styleId="prtyId"  >
 <html:option value="0">---select---</html:option>    
</html:select>
</td>
</tr>
<tr>
<td>Terms :</td><td>
<html:select property="relIdn" name="mixDeliveryForm"  onchange="getSaleIdnMix('SL')" styleId="rlnId"  >
     <html:option value="0">---select---</html:option>
</html:select>
...
<div id="memoIdn"></div>
    ...
    <td nowrap="nowrap"><html:text property="<%=field%>" size="6" styleId="<%=typ%>" onchange="<%=onchang%>" name="mixDeliveryForm"/></td>
    <td nowrap="nowrap">
    <%if(rmk.equals("Y")){%>
    Remark<html:text property="<%=fieldRmk%>" styleId="<%=fieldId%>" onchange="<%=onchangrmk%>" name="mixDeliveryForm" size="10"/>
    <%}%>
    </td>
    </tr>
    <%}else{%>
    ...
    <td nowrap="nowrap"><b><span id="<%=typ%>_dis"></span></b></td>
    <td nowrap="nowrap">
    <html:hidden property="<%=field%>"  styleId="<%=typ%>" name="mixDeliveryForm"/>
...
<table>
 <html:form action="/mixAkt/mixDeliveryAction.do?method=save" method="POST"  onsubmit="return confirmChanges()">
   ...
   if(request.getAttribute("view")!=null && view.equals("Y")){
    ...
    %>
    <html:hidden property="<%=field%>" name="mixDeliveryForm" styleId="<%=fieldId%>"  />
   <%if(rmk.equals("Y")){%>
    <html:hidden property="<%=fieldRmk%>" name="mixDeliveryForm" styleId="<%=fieldIdrmk%>"  />
    <%}
    if(flg.equals("AUTO") && autoopt.equals("OPT")){
    String chk="N";
    if(setcharge.contains(typ))
    chk="Y";%>
    <html:hidden property="<%=fieldauto%>" name="mixDeliveryForm" styleId="<%=fieldIdauto%>" value="<%=chk%>"  />
    <%}
    }%>
   <html:hidden property="value(vluamt)" name="mixDeliveryForm" styleId="vluamt"  />
    <%}}
    %>
   <tr><td>
   ...
   <table>
          ...
          <td><b>Email :</b></td> <td> <bean:write property="value(email)" name="mixDeliveryForm"/> </td>
          <td><b>Mobile No :</b></td> <td> <bean:write property="value(mobile)" name="mixDeliveryForm"/> </td>
          <td><b>Office No :</b></td> <td> <bean:write property="value(office)" name="mixDeliveryForm"/> </td>
   ...
   <tr><td>
          ...
          <td>
           <html:select property="value(grpIdn)" name="mixDeliveryForm"  styleId="grpIdn"  >
            ...
            <td> <html:select property="value(bankAddr)" name="mixDeliveryForm" style="dispaly:none" styleId="bankAddr">
            ...
            <td><html:radio property="value(aadatpaid)" onchange="setBrokerComm('aadatcomm','Y')" styleId="aadatpaid1"  value="Y"  name="mixDeliveryForm"/> </td>
            <td>&nbsp;Yes</td> 
            <td><html:radio property="value(aadatpaid)"  onchange="setBrokerComm('aadatcomm','N')" styleId="aadatpaid2" value="N" name="mixDeliveryForm"/></td>
            <td>&nbsp;No</td>
           ...
           <td id="brk1Nme"><bean:write property="value(brk1)" name="mixDeliveryForm" /></td><td><html:radio property="value(brk1paid)" styleId="brk1paid1" value="Y" name="mixDeliveryForm"/></td><td>&nbsp;Yes</td>  <td><html:radio property="value(brk1paid)"  styleId="brk1paid2"  value="N" name="mixDeliveryForm"/></td><td>&nbsp;No</td> <td><html:text property="value(brk1comm)"  styleId="brk1comm"  readonly="true" size="3" name="mixDeliveryForm" /> </td>
           </tr>
           </table>
           </div>
           </td>
           ...
           <table>
           <tr>
           <td id="brk2Nme"><bean:write property="value(brk2)" name="mixDeliveryForm" /></td><td><html:radio property="value(brk2paid)"  styleId="brk2paid1" value="Y" name="mixDeliveryForm"/> </td><td>&nbsp;Yes</td> <td><html:radio property="value(brk2paid)"  styleId="brk2paid2"   value="N" name="mixDeliveryForm"/></td><td> &nbsp;No</td> <td><html:text property="value(brk2comm)" styleId="brk2comm" readonly="true" size="3" name="mixDeliveryForm" /> </td>
           </tr>
           </table>
           </div>
           </td>
           ...
           <table>
           <tr>
           <td id="brk3Nme"><bean:write property="value(brk3)" name="mixDeliveryForm" /></td><td><html:radio property="value(brk3paid)"   styleId="brk3paid1" value="Y" name="mixDeliveryForm"/> </td><td>&nbsp;Yes</td> <td><html:radio property="value(brk3paid)"  styleId="brk3paid2" value="N" name="mixDeliveryForm"/></td><td> &nbsp;No</td>  <td><html:text property="value(brk3comm)" styleId="brk3comm" readonly="true" size="3" name="mixDeliveryForm" /> </td>
           </tr>
           </table>
           </div>
           </td>
           ...
           <table>
           <tr>
           <td id="brk4Nme"><bean:write property="value(brk4)" name="mixDeliveryForm" /></td><td><html:radio property="value(brk4paid)"   styleId="brk4paid1"  value="Y" name="mixDeliveryForm"/> </td><td>&nbsp;Yes</td> <td><html:radio property="value(brk4paid)"   styleId="brk4paid2" value="N" name="mixDeliveryForm"/></td><td> &nbsp;No</td> <td><html:text property="value(brk4comm)" styleId="brk4comm" readonly="true" size="3" name="mixDeliveryForm" /> </td>
           </tr>
           </table>
           </div>
           </td>
  ...
  <tr><td>
        ...
        %>          
          <input type="hidden" id="inc_vlu_<%=pktIdn%>" value="<%=pkt.getInc_vlu()%>" />      
            ...
            <%}}%>
            <td>
            <% String fldNme = "SR_"+saleIdn+"_"+pktIdn;
            String fldNmeId = count+"_SR";
            String onchange="MixDlvCalCulation()";
            %>
            <input type="text" name="<%=fldNme%>" id="<%=fldNmeId%>" onchange="<%=onchange%>" size="4" />
            </td>
                </tr>
            ...
            <table class="grid1">
             <tr><th>Sr.</th><th>Qty</th><th>Total Carat</th><th>Avg Rate</th><th>Description</th>
            ...
            <th> <%=lprp%></th>
             <%}%>
                ...
                <%for(int i=1;i<=20;i++){
                      String txtNme="DSC_"+i;
                      String ctsNme="CRTWT_"+i;
                      String avgNme="RTE_"+i;
                      String qtyNme="QTY_"+i;
                      %>
                  <tr>
                 <td><%=i%> </td><td><input type="text" name="<%=qtyNme%>" id="<%=qtyNme%>" size="3"  />  </td>
                <td><input type="text" name="<%=ctsNme%>" id="<%=ctsNme%>" size="5"  />  </td><td><input type="text" name="<%=avgNme%>" id="<%=avgNme%>" size="10" />  </td>
                    <td><input type="text" name="<%=txtNme%>" id="<%=txtNme%>" />  </td>
                     <%for(int j=0;j<vwPrpLst.size();j++){
                String lprp = (String)vwPrpLst.get(j);
                String fldId = lprp+"_"+i;
                 String fldNme = "value("+lprp+"_"+i+")";
                ArrayList prpLst = (ArrayList)prp.get(lprp+"V");
              if(prpLst!=null && prpLst.size()>0){%>
          <td>     <html:select property="<%=fldNme%>"  name="mixDeliveryForm"   >
              <%  for(int x=0;x<prpLst.size();x++){
                String lVal  = (String)prpLst.get(x);
                  
                 
                %>
              <html:option value="<%=lVal%>" ><%=lVal%></html:option>
                 <%}%></html:select></td>
