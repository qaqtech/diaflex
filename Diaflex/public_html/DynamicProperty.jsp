...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
  <%
   ArrayList params = new ArrayList();
    ...
    String keyNme = util.nvl((String)info.getValue("mdl"));
    if(pgTtl.indexOf("~") > -1)
    ...
    String formAction = "/dynamicProperty.do?method=save";
 %>
 <tr>
...
<%}%>
  ...
  <html:hidden property="value(mdl)" />
  <html:hidden property="value(sname)" />
  <html:hidden property="value(par)" />
    <table class="grid1" id="tb">
...
<%
    ...
    for(int dsp=0; dsp < dspOrder.size(); dsp++) {
      ...
      %>
        <html:checkbox property="<%=cboxPrp%>"  name="dynamicPropertyForm"/>
      <%}%>
            </td>
      <%
        for(int j=0; j < daos.size(); j++) {
            ...
            if(htmlFld.indexOf("srt_") > -1)
               valCond="validateRepPrpSort('"+htmlFld+"')";
        %>
        <td>
        <%
            if(fldTyp.equals("T")) {
            %>
                <html:text property="<%=fldNm%>" size="<%=fldSz%>" styleId="<%=htmlFld%>" onchange="<%=valCond%>" name="dynamicPropertyForm" />
            <%}
            if((fldTyp.equals("S")) || (fldTyp.equals("L"))){
                ...
                %>
                <html:select property="<%=fldNm%>"  name="dynamicPropertyForm">
             ...
             if(fldTyp.equals("DT")){%>
            <div class="float">   <html:text property="<%=fldNm%>"  name="dynamicPropertyForm"  styleId="<%=htmlFld%>" /></div>
