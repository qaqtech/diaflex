...
<html>
 ...
 <body onfocus="<%=onfocus%>">
  ...
  HashMap pricePrpDtl=(HashMap)session.getAttribute("pricePrpDtl");
   ...
   String lprp="";
  %>
    <html:form  action="/Inward/transferMktSH.do?method=saveTrfMkt">
    <input type="hidden" name="rap_rte" value="<%=rap_rte%>" id="rap_rte" />
    <%for(int g=0;g < grpList.size();g++){
    String grp=(String)grpList.get(g);
    String labname="value(lab_"+grp+")";
    %>
  <html:hidden property="<%=labname%>" name="transferToMktSHForm" />
  <%}%>
    <html:hidden property="value(mstkIdn)" name="transferToMktSHForm" />
     <html:hidden property="value(mdl)" name="transferToMktSHForm" />
    <html:hidden property="value(issueId)" name="transferToMktSHForm" />
    <html:hidden property="value(lastpage)" name="transferToMktSHForm" styleId="lastpage" />
     <html:hidden property="value(currentpage)" name="transferToMktSHForm" styleId="currentpage" />
    <html:hidden property="value(url)" styleId="url" name="transferToMktSHForm"  />
  ...
  <tr>
  <td> <span class="txtLink" > <bean:write property="value(vnm)" name="transferToMktSHForm" /></span>&nbsp;
  <html:submit property="submit" value="Update Stock Properties " styleClass="submit"/>
  <%
  ...
  <tr><td>
<logic:present name="transferToMktSHForm" property="msg">
<span class="redLabel"> <bean:write name="transferToMktSHForm" property="msg" /></span>
</logic:present>
&nbsp;
  </td></tr>
  <tr><td>
...
<%if(prpUpdList.contains(lprp) && (grp.equals("1") || cnt.equals("hk"))){
       ...
       String  fldName = "value("+lprp+"_"+grp+")";
       String fldId=lprp+"_"+grp;
       if(typ.equals("C")){
         ArrayList prpVal = (ArrayList)prp.get(lprp+"V");
  if(prpVal!=null){
  %>
  <html:select property="<%=fldName%>" styleId="<%=fldId%>" name="transferToMktSHForm" >
...
//      
  ...
  if(!getRtnPrp.equals("")){
  if(prpUpdList.contains(getRtnPrp) && viewPrp.contains(getRtnPrp)){
  onchange="getviceversaRTEDIS('"+lprp+"','"+getRtnPrp+"','"+grp+"')";
  }
  }
  %>
   <html:text property="<%=fldName%>" styleId="<%=fldId%>" onchange="<%=onchange%>" size="4" name="transferToMktSHForm"/>
   <%}else if(typ.equals("D")){%>
  <html:text property="<%=fldName%>" styleId="<%=lprp%>"  readonly="true"  name="transferToMktSHForm" size="11" /><img src="<%=info.getReqUrl()%>/images/calender.png" onClick="setYears(2011, <%=info.getCurrentYear()%>);showCalender(this, '<%=lprp%>');"> 
  <%} else{%>
  <html:text property="<%=fldName%>" size="15" name="transferToMktSHForm"/>
  <%}%>
       <%}else{%>
        <%=lprpVal%>
       <%}%>
