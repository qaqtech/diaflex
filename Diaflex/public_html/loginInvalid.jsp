...
<html>
  <head>
    ...
    <title>Diaflex : Invalidate</title>
        <%
    ...
    System.out.println("Diaflex : Invalidate"+logoutlogIdn);
    %>
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="mainbg" height="600">
  ...
  if(msg.equals("")){
  %>
  <label class="pgTtl">Current session is logged out, as user '<%=usr%>' has logged in from other machine/browser</label>
  <%}else{%>
  <label class="pgTtl">'<%=msg%>'</label>
  <%}%>
